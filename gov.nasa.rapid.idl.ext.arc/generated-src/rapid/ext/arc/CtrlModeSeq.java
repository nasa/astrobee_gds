...
/**
* A sequence of CtrlMode instances.
*/
public final class CtrlModeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CtrlModeSeq() {
        super(CtrlMode.class);
    }

    public CtrlModeSeq (int initialMaximum) {
        super(CtrlMode.class, initialMaximum);
    }

    public CtrlModeSeq (Collection elements) {
        super(CtrlMode.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CtrlMode.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CtrlMode objects (beyond the original size of this object)
