...
/**
* A sequence of OctetSequence170K instances.
*/
public final class OctetSequence170KSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OctetSequence170KSeq() {
        super(OctetSequence170K.class);
    }

    public OctetSequence170KSeq (int initialMaximum) {
        super(OctetSequence170K.class, initialMaximum);
    }

    public OctetSequence170KSeq (Collection elements) {
        super(OctetSequence170K.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OctetSequence170K.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OctetSequence170K objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OctetSequence170K.create());
