...
/**
* A sequence of GeometryIndexMode instances.
*/
public final class GeometryIndexModeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GeometryIndexModeSeq() {
        super(GeometryIndexMode.class);
    }

    public GeometryIndexModeSeq (int initialMaximum) {
        super(GeometryIndexMode.class, initialMaximum);
    }

    public GeometryIndexModeSeq (Collection elements) {
        super(GeometryIndexMode.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GeometryIndexMode.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GeometryIndexMode objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GeometryIndexMode.create());
