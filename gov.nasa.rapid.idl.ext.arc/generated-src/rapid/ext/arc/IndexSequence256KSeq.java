...
/**
* A sequence of IndexSequence256K instances.
*/
public final class IndexSequence256KSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public IndexSequence256KSeq() {
        super(IndexSequence256K.class);
    }

    public IndexSequence256KSeq (int initialMaximum) {
        super(IndexSequence256K.class, initialMaximum);
    }

    public IndexSequence256KSeq (Collection elements) {
        super(IndexSequence256K.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, IndexSequence256K.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' IndexSequence256K objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(IndexSequence256K.create());
