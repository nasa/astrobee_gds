...
/**
* A sequence of EphemerisConfigVector32 instances.
*/
public final class EphemerisConfigVector32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EphemerisConfigVector32Seq() {
        super(EphemerisConfigVector32.class);
    }

    public EphemerisConfigVector32Seq (int initialMaximum) {
        super(EphemerisConfigVector32.class, initialMaximum);
    }

    public EphemerisConfigVector32Seq (Collection elements) {
        super(EphemerisConfigVector32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EphemerisConfigVector32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EphemerisConfigVector32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EphemerisConfigVector32.create());
