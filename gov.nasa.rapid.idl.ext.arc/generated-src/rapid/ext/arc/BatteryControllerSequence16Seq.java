...
/**
* A sequence of BatteryControllerSequence16 instances.
*/
public final class BatteryControllerSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryControllerSequence16Seq() {
        super(BatteryControllerSequence16.class);
    }

    public BatteryControllerSequence16Seq (int initialMaximum) {
        super(BatteryControllerSequence16.class, initialMaximum);
    }

    public BatteryControllerSequence16Seq (Collection elements) {
        super(BatteryControllerSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryControllerSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryControllerSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryControllerSequence16.create());
