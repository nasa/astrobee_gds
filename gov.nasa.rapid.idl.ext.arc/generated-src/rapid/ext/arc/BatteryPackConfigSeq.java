...
/**
* A sequence of BatteryPackConfig instances.
*/
public final class BatteryPackConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryPackConfigSeq() {
        super(BatteryPackConfig.class);
    }

    public BatteryPackConfigSeq (int initialMaximum) {
        super(BatteryPackConfig.class, initialMaximum);
    }

    public BatteryPackConfigSeq (Collection elements) {
        super(BatteryPackConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryPackConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryPackConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryPackConfig.create());
