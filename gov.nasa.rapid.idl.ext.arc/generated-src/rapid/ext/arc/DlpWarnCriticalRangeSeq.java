...
/**
* A sequence of DlpWarnCriticalRange instances.
*/
public final class DlpWarnCriticalRangeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DlpWarnCriticalRangeSeq() {
        super(DlpWarnCriticalRange.class);
    }

    public DlpWarnCriticalRangeSeq (int initialMaximum) {
        super(DlpWarnCriticalRange.class, initialMaximum);
    }

    public DlpWarnCriticalRangeSeq (Collection elements) {
        super(DlpWarnCriticalRange.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DlpWarnCriticalRange.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DlpWarnCriticalRange objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DlpWarnCriticalRange.create());
