...
/**
* A sequence of TrajPoint3DSequence instances.
*/
public final class TrajPoint3DSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TrajPoint3DSequenceSeq() {
        super(TrajPoint3DSequence.class);
    }

    public TrajPoint3DSequenceSeq (int initialMaximum) {
        super(TrajPoint3DSequence.class, initialMaximum);
    }

    public TrajPoint3DSequenceSeq (Collection elements) {
        super(TrajPoint3DSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TrajPoint3DSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TrajPoint3DSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TrajPoint3DSequence.create());
