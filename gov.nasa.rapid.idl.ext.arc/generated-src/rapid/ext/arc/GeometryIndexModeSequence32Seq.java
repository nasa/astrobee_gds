...
/**
* A sequence of GeometryIndexModeSequence32 instances.
*/
public final class GeometryIndexModeSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GeometryIndexModeSequence32Seq() {
        super(GeometryIndexModeSequence32.class);
    }

    public GeometryIndexModeSequence32Seq (int initialMaximum) {
        super(GeometryIndexModeSequence32.class, initialMaximum);
    }

    public GeometryIndexModeSequence32Seq (Collection elements) {
        super(GeometryIndexModeSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GeometryIndexModeSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GeometryIndexModeSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GeometryIndexModeSequence32.create());
