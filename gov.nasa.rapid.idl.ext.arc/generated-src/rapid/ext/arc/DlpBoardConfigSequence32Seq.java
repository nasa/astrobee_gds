...
/**
* A sequence of DlpBoardConfigSequence32 instances.
*/
public final class DlpBoardConfigSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DlpBoardConfigSequence32Seq() {
        super(DlpBoardConfigSequence32.class);
    }

    public DlpBoardConfigSequence32Seq (int initialMaximum) {
        super(DlpBoardConfigSequence32.class, initialMaximum);
    }

    public DlpBoardConfigSequence32Seq (Collection elements) {
        super(DlpBoardConfigSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DlpBoardConfigSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DlpBoardConfigSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DlpBoardConfigSequence32.create());
