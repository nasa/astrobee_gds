...
/**
* A sequence of GeometryMeshSample instances.
*/
public final class GeometryMeshSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GeometryMeshSampleSeq() {
        super(GeometryMeshSample.class);
    }

    public GeometryMeshSampleSeq (int initialMaximum) {
        super(GeometryMeshSample.class, initialMaximum);
    }

    public GeometryMeshSampleSeq (Collection elements) {
        super(GeometryMeshSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GeometryMeshSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GeometryMeshSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GeometryMeshSample.create());
