...
/**
* A sequence of GeometryAppearanceState instances.
*/
public final class GeometryAppearanceStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GeometryAppearanceStateSeq() {
        super(GeometryAppearanceState.class);
    }

    public GeometryAppearanceStateSeq (int initialMaximum) {
        super(GeometryAppearanceState.class, initialMaximum);
    }

    public GeometryAppearanceStateSeq (Collection elements) {
        super(GeometryAppearanceState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GeometryAppearanceState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GeometryAppearanceState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GeometryAppearanceState.create());
