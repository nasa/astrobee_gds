...
/**
* A sequence of MotorStateSequence32 instances.
*/
public final class MotorStateSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MotorStateSequence32Seq() {
        super(MotorStateSequence32.class);
    }

    public MotorStateSequence32Seq (int initialMaximum) {
        super(MotorStateSequence32.class, initialMaximum);
    }

    public MotorStateSequence32Seq (Collection elements) {
        super(MotorStateSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MotorStateSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MotorStateSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MotorStateSequence32.create());
