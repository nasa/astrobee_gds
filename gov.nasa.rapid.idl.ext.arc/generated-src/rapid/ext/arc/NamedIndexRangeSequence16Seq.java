...
/**
* A sequence of NamedIndexRangeSequence16 instances.
*/
public final class NamedIndexRangeSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedIndexRangeSequence16Seq() {
        super(NamedIndexRangeSequence16.class);
    }

    public NamedIndexRangeSequence16Seq (int initialMaximum) {
        super(NamedIndexRangeSequence16.class, initialMaximum);
    }

    public NamedIndexRangeSequence16Seq (Collection elements) {
        super(NamedIndexRangeSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedIndexRangeSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedIndexRangeSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedIndexRangeSequence16.create());
