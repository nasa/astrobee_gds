...
/**
* A sequence of StateMachineState instances.
*/
public final class StateMachineStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public StateMachineStateSeq() {
        super(StateMachineState.class);
    }

    public StateMachineStateSeq (int initialMaximum) {
        super(StateMachineState.class, initialMaximum);
    }

    public StateMachineStateSeq (Collection elements) {
        super(StateMachineState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, StateMachineState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' StateMachineState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(StateMachineState.create());
