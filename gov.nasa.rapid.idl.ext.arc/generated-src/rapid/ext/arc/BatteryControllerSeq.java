...
/**
* A sequence of BatteryController instances.
*/
public final class BatteryControllerSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryControllerSeq() {
        super(BatteryController.class);
    }

    public BatteryControllerSeq (int initialMaximum) {
        super(BatteryController.class, initialMaximum);
    }

    public BatteryControllerSeq (Collection elements) {
        super(BatteryController.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryController.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryController objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryController.create());
