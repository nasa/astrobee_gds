...
/**
* A sequence of TransitionSequence16K instances.
*/
public final class TransitionSequence16KSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public TransitionSequence16KSeq() {
        super(TransitionSequence16K.class);
    }

    public TransitionSequence16KSeq (int initialMaximum) {
        super(TransitionSequence16K.class, initialMaximum);
    }

    public TransitionSequence16KSeq (Collection elements) {
        super(TransitionSequence16K.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, TransitionSequence16K.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' TransitionSequence16K objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(TransitionSequence16K.create());
