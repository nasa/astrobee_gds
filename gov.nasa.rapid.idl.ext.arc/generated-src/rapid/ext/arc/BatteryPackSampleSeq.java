...
/**
* A sequence of BatteryPackSample instances.
*/
public final class BatteryPackSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryPackSampleSeq() {
        super(BatteryPackSample.class);
    }

    public BatteryPackSampleSeq (int initialMaximum) {
        super(BatteryPackSample.class, initialMaximum);
    }

    public BatteryPackSampleSeq (Collection elements) {
        super(BatteryPackSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryPackSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryPackSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryPackSample.create());
