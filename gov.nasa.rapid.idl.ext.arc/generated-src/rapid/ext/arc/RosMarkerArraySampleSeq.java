...
/**
* A sequence of RosMarkerArraySample instances.
*/
public final class RosMarkerArraySampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RosMarkerArraySampleSeq() {
        super(RosMarkerArraySample.class);
    }

    public RosMarkerArraySampleSeq (int initialMaximum) {
        super(RosMarkerArraySample.class, initialMaximum);
    }

    public RosMarkerArraySampleSeq (Collection elements) {
        super(RosMarkerArraySample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RosMarkerArraySample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RosMarkerArraySample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RosMarkerArraySample.create());
