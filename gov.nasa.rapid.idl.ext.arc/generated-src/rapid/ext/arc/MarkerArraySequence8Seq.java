...
/**
* A sequence of MarkerArraySequence8 instances.
*/
public final class MarkerArraySequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MarkerArraySequence8Seq() {
        super(MarkerArraySequence8.class);
    }

    public MarkerArraySequence8Seq (int initialMaximum) {
        super(MarkerArraySequence8.class, initialMaximum);
    }

    public MarkerArraySequence8Seq (Collection elements) {
        super(MarkerArraySequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MarkerArraySequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MarkerArraySequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MarkerArraySequence8.create());
