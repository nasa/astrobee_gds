...
/**
* A sequence of EphemerisConfigEntry instances.
*/
public final class EphemerisConfigEntrySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EphemerisConfigEntrySeq() {
        super(EphemerisConfigEntry.class);
    }

    public EphemerisConfigEntrySeq (int initialMaximum) {
        super(EphemerisConfigEntry.class, initialMaximum);
    }

    public EphemerisConfigEntrySeq (Collection elements) {
        super(EphemerisConfigEntry.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EphemerisConfigEntry.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EphemerisConfigEntry objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(EphemerisConfigEntry.create());
