...
public class GpsSampleTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "rapid::ext::arc::GpsSample";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final GpsSampleTypeSupport _singleton
    = new GpsSampleTypeSupport();
    ...
    */
    public static GpsSampleTypeSupport get_instance() {
        return _singleton;
    }

    public static GpsSampleTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return GpsSampleTypeCode.VALUE;
    }

    public Object create_data() {
        return GpsSample.create();
    }
    ...
    * <code>this</code> is not a <code>GpsSample</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        GpsSample typedDst = (GpsSample) destination;
        GpsSample typedSrc = (GpsSample) source;
    ...
    public long get_serialized_sample_size(
        ...
        Object sample) 
    {
        GpsSample typedSrc = (GpsSample) sample;
        long origAlignment = currentAlignment;
        long encapsulation_size = currentAlignment;
        ...
        currentAlignment += rapid.Vec3dTypeSupport.get_instance().get_serialized_sample_size(
            endpoint_data,false,encapsulation_id,currentAlignment,typedSrc.sigmaXyz);
        currentAlignment  +=  CdrPrimitiveType.BYTE.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.BYTE.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.BYTE.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.BYTE.getMaxSizeSerialized(currentAlignment);
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            rapid.Vec3dTypeSupport.get_instance().serialize(endpoint_data, typedSrc.sigmaXyz, dst, false, encapsulation_id,true,endpoint_plugin_qos);

            dst.writeByte(typedSrc.utmZone);

            dst.writeChar(typedSrc.utmDesig);

            dst.writeByte(typedSrc.mode);

            dst.writeByte(typedSrc.numSats);

            dst.writeLong(typedSrc.diffAge);

            dst.writeLong(typedSrc.solAge);

            dst.writeFloat(typedSrc.undulation);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            GpsSample typedDst = (GpsSample) dst;
            typedDst.clear();      
            ...
            try{
                ...
                typedDst.sigmaXyz = (rapid.Vec3d)rapid.Vec3dTypeSupport.get_instance().deserialize_sample(endpoint_data, typedDst.sigmaXyz, src, false, true, endpoint_plugin_qos);     
                typedDst.utmZone = src.readByte();
                typedDst.utmDesig = src.readChar();
                typedDst.mode = src.readByte();
                typedDst.numSats = src.readByte();
                typedDst.diffAge = src.readLong();
                typedDst.solAge = src.readLong();
                typedDst.undulation = src.readFloat();
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if (deserialize_key) {
            ...
            GpsSample typedDst = (GpsSample) sample;
    ...
    */
    ...
    int mask) {
        return new GpsSampleDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new GpsSampleDataReader(native_reader, listener, mask, this);   
    ...
    protected GpsSampleTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME,true,GpsSampleTypeCode.VALUE,GpsSample.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected GpsSampleTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,GpsSampleTypeCode.VALUE,GpsSample.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
