...
/**
* A sequence of StateMachineConfig instances.
*/
public final class StateMachineConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public StateMachineConfigSeq() {
        super(StateMachineConfig.class);
    }

    public StateMachineConfigSeq (int initialMaximum) {
        super(StateMachineConfig.class, initialMaximum);
    }

    public StateMachineConfigSeq (Collection elements) {
        super(StateMachineConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, StateMachineConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' StateMachineConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(StateMachineConfig.create());
