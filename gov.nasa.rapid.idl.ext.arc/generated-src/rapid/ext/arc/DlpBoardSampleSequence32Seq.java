...
/**
* A sequence of DlpBoardSampleSequence32 instances.
*/
public final class DlpBoardSampleSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DlpBoardSampleSequence32Seq() {
        super(DlpBoardSampleSequence32.class);
    }

    public DlpBoardSampleSequence32Seq (int initialMaximum) {
        super(DlpBoardSampleSequence32.class, initialMaximum);
    }

    public DlpBoardSampleSequence32Seq (Collection elements) {
        super(DlpBoardSampleSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DlpBoardSampleSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DlpBoardSampleSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DlpBoardSampleSequence32.create());
