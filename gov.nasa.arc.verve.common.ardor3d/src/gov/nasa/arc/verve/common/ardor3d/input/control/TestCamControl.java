 ...
 *
 */
public class TestCamControl extends AbstractCamControl implements InterestPointListener {
    static Logger logger = Logger.getLogger(TestCamControl.class.getName());
    ...
    //=========================================================================
    public TestCamControl(final IVerveCanvasView canvasView, final ReadOnlyVector3 upAxis) {
    ...
    public void handleFrameUpdate(double tpf) {
        ...
        //logger.debug("diff="+Ardor3D.format(diff));
    ...
    InputTrigger createDraggedInputTrigger() {
        final TestCamControl control = this;
    ...
    InputTrigger createRightClickInputTrigger() {
        ...
        return new InputTrigger(new MouseButtonClickedCondition(MouseButton.RIGHT), rightClickAction);
    }
    ...
    InputTrigger createMiddleClickTrigger() {
        ...
        final TriggerAction clickAction = new TriggerAction() {
            @Override
            public void perform(Canvas source, TwoInputStates inputStates, double tpf) {
                if( source.equals(TestCamControl.this.getCanvas()) ) {
                    final MouseState mouse = inputStates.getCurrent().getMouseState();
                    TestCamControl.this.middleMouseClick(mouse);
                }
            }
        };
    ...
    InputTrigger createMouseReleasedTrigger() {
        final TriggerAction triggerAction = new TriggerAction() {
            ...
            public void perform(Canvas source, TwoInputStates inputStates, double tpf) {
                if( source.equals(TestCamControl.this.getCanvas()) ) {
                    final MouseState mouse = inputStates.getCurrent().getMouseState();
                    TestCamControl.this.mouseRelease(mouse);
                }
            }
        };
