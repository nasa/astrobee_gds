...
public class ParallelSplitShadowMapPass extends Pass {
    private static final Logger logger = Logger.getLogger(ParallelSplitShadowMapPass.class);
    ...
    public void init(final Renderer r) {
        ...
        if (caps.isGLSLSupported()) {
            ...
            try {
                _pssmShader.setVertexShader(ResourceLocatorTool.getClassPathResourceAsStream( ParallelSplitShadowMapPass.class, 
                        "gov/nasa/arc/verve/common/ardor3d/pssm/pssm.vert"));
                if (filter == Filter.None) {
                    _pssmShader.setFragmentShader(ResourceLocatorTool.getClassPathResourceAsStream(ParallelSplitShadowMapPass.class, 
                            "gov/nasa/arc/verve/common/ardor3d/pssm/pssm.frag"));
                } 
                else if (filter == Filter.Pcf) {
                    _pssmShader.setFragmentShader(ResourceLocatorTool.getClassPathResourceAsStream(ParallelSplitShadowMapPass.class, 
                            "gov/nasa/arc/verve/common/ardor3d/pssm/pssmPCF.frag"));
                }
            } catch (final IOException ex) {
                logger.error("init(Renderer) - Could not load shaders.", ex);
            }
            _mainShader = _pssmShader;

            _pssmDebugShader = new GLSLShaderObjectsState();
            try {
                _pssmDebugShader.setVertexShader(ResourceLocatorTool.getClassPathResourceAsStream(ParallelSplitShadowMapPass.class, 
                        "gov/nasa/arc/verve/common/ardor3d/pssm/pssmDebug.vert"));
                _pssmDebugShader.setFragmentShader(ResourceLocatorTool.getClassPathResourceAsStream(ParallelSplitShadowMapPass.class, 
                        "gov/nasa/arc/verve/common/ardor3d/pssm/pssmDebug.frag"));
            } catch (final IOException ex) {
                logger.error("init(Renderer) - Could not load shaders.", ex);
            }
        }
    ...
    protected void doRender(final Renderer r) {
        ...
        //r.clearBuffers(Renderer.BUFFER_DEPTH);
    ...
    public void updateShadowMaps(final Renderer r) {
        ...
        if (_updateMainCamera) {
            ...
            if (hasValidBounds) {
                //logger.info("_receiverBounds.getCenter()="+_receiverBounds.getCenter());
                //logger.info("_receiverBounds.getRadius()="+_receiverBounds.getRadius());
                _pssmCam.pack(_receiverBounds);
            } else {
                _pssmCam.updateMaxCameraFar();
            }

            //logger.debug("hasValidBounds=" + hasValidBounds);
        }
    ...
    private Matrix3 viewMat = new Matrix3();
    private Vector3 negLeft = new Vector3();
    private Vector3 negDir  = new Vector3();
    private void updateShaderVariables() {
        if (_pssmShader != null && ContextManager.getCurrentContext().getCapabilities().isGLSLSupported()) {
            ...
            if (!_drawShaderDebug) {
                ...
                Vector3          lightDir = light.getDirection().normalize(new Vector3());
                viewMat.setColumn(0, cam.getLeft().negate(negLeft));
                viewMat.setColumn(1, cam.getUp());
                viewMat.setColumn(2, cam.getDirection().negate(negDir));
                viewMat.applyPre(lightDir, lightDir);
                currentShader.setUniform("lightVec", lightDir);
            }
    ...
    private void updateShadowMap(final int index, final Renderer r) {
        ...
        for(WeakReference<Spatial> ref : ShadowCasters.INSTANCE.getSpatialRefs()) {
    ...
    private void updateReceiverBounds() {
        ...
        //logger.debug("  _boundsReceiver.size="+_boundsReceiver.size ());
        for (int i = 0, cSize = _boundsReceiver.size(); i < cSize; i++) {
            ...
            if (child != null && child.getSceneHints().getCullHint() != CullHint.Always && child.getWorldBound() != null && boundIsValid(child.getWorldBound())) {
                if (firstRun) {
        ...
        //logger.debug("  _spatials.size="+_spatials.size ());
        for (int i = 0, cSize = _spatials.size(); i < cSize; i++) {
            final Spatial child = _spatials.get(i);
            if(child.getWorldBound() != null) {
                if(false) { //!boundIsValid(child.getWorldBound())) {
                    CheckBoundsVisitor visitor = new CheckBoundsVisitor();
                    child.acceptVisitor(visitor, false);
                }
            }
            //logger.debug("         worldBound.isValid ="+boundIsValid(child.getWorldBound()));
            if (child != null && child.getSceneHints().getCullHint() != CullHint.Always && child.getWorldBound() != null && boundIsValid(child.getWorldBound())) {
                if (firstRun) {
                    _receiverBounds.setCenter(child.getWorldBound().getCenter());
                    _receiverBounds.setXExtent(0);
    ...
    public void setNumOfSplits(final int numOfSplits) {
        ...
        if (_numOfSplits != numOfSplits) {
            logger.warn("Valid range for number of splits is " + _MIN_SPLITS + " to " + _MAX_SPLITS
