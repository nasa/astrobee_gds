 ...
 *
 */
public class NadirCamControl extends AbstractCamControl {
    static Logger logger = Logger.getLogger(NadirCamControl.class.getName());

    Spatial       m_spatial       = null;
    final Vector3 m_spatialOffset = new Vector3(0,0,0);
    final Vector3 m_spatialVec    = new Vector3();

    final Vector3 m_actualCtr     = new Vector3(0,0,0);
    final Vector3 m_targetCtr     = new Vector3(0,0,0);

    final Vector3 m_actualLoc     = new Vector3(0,0,20);
    double        m_targetDistance= 10;
    double        m_distance      = m_targetDistance;

    double        m_targetYaw     = 0;
    double        m_actualYaw     = 0;
    ...
    double        m_crabSpeed     = 0.001; 

    static int instanceCount = 0;
    public final int instance;

    final Vector3 m_camUp   = new Vector3();
    final Vector3 m_camLeft = new Vector3();
    final Vector3 m_camLook = new Vector3();
     ...
     * @param scene
     */
    public NadirCamControl(final IVerveCanvasView canvasView, final ReadOnlyVector3 upAxis) {
    ...
    public void reset() {
        ...
        m_actualCtr.set(m_targetCtr.set(0,0,z));
        m_actualLoc.set(0,0,z+100);
        m_targetDistance= 10;
    ...
    public void reinit(AbstractCamControl pcc) {
        if(pcc != this) {
            ...
            if(pcc == null) {
                ...
                m_actualLoc.set(m_actualCtr.getX(), m_actualCtr.getY(), m_actualCtr.getZ()+m_distance);
    ...
    InputTrigger createDraggedInputTrigger() {
        final NadirCamControl control = this;
        ...
        final TriggerAction leftClickAction = new TriggerAction() {
            ...
            public void perform(Canvas source, TwoInputStates inputStates, double tpf) {
                if( source.equals(control.getCanvas()) ) {
                    final MouseState mouse = inputStates.getCurrent().getMouseState();
                    final KeyboardState keybd = inputStates.getCurrent().getKeyboardState();
                    boolean mod = keybd.isDown(Key.LSHIFT) || keybd.isDown(Key.RSHIFT);
                    control.leftMouseClick(mouse, mod);
                }
            }
        };
