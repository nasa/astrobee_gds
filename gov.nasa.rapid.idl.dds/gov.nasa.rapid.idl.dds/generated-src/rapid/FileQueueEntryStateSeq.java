...
/**
* A sequence of FileQueueEntryState instances.
*/
public final class FileQueueEntryStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FileQueueEntryStateSeq() {
        super(FileQueueEntryState.class);
    }

    public FileQueueEntryStateSeq (int initialMaximum) {
        super(FileQueueEntryState.class, initialMaximum);
    }

    public FileQueueEntryStateSeq (Collection elements) {
        super(FileQueueEntryState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FileQueueEntryState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FileQueueEntryState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FileQueueEntryState.create());
