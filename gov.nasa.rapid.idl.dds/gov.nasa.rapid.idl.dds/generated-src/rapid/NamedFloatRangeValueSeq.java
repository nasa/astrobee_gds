...
/**
* A sequence of NamedFloatRangeValue instances.
*/
public final class NamedFloatRangeValueSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedFloatRangeValueSeq() {
        super(NamedFloatRangeValue.class);
    }

    public NamedFloatRangeValueSeq (int initialMaximum) {
        super(NamedFloatRangeValue.class, initialMaximum);
    }

    public NamedFloatRangeValueSeq (Collection elements) {
        super(NamedFloatRangeValue.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedFloatRangeValue.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedFloatRangeValue objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedFloatRangeValue.create());
