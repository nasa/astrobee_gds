...
/**
* A sequence of KeyTypeValueSequence64 instances.
*/
public final class KeyTypeValueSequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeValueSequence64Seq() {
        super(KeyTypeValueSequence64.class);
    }

    public KeyTypeValueSequence64Seq (int initialMaximum) {
        super(KeyTypeValueSequence64.class, initialMaximum);
    }

    public KeyTypeValueSequence64Seq (Collection elements) {
        super(KeyTypeValueSequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeValueSequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeValueSequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeValueSequence64.create());
