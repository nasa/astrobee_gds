...
/**
* A sequence of LongLongSequence128 instances.
*/
public final class LongLongSequence128Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LongLongSequence128Seq() {
        super(LongLongSequence128.class);
    }

    public LongLongSequence128Seq (int initialMaximum) {
        super(LongLongSequence128.class, initialMaximum);
    }

    public LongLongSequence128Seq (Collection elements) {
        super(LongLongSequence128.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LongLongSequence128.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LongLongSequence128 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LongLongSequence128.create());
