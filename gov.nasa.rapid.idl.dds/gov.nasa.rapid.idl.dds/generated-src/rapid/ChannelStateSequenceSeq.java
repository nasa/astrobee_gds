...
/**
* A sequence of ChannelStateSequence instances.
*/
public final class ChannelStateSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ChannelStateSequenceSeq() {
        super(ChannelStateSequence.class);
    }

    public ChannelStateSequenceSeq (int initialMaximum) {
        super(ChannelStateSequence.class, initialMaximum);
    }

    public ChannelStateSequenceSeq (Collection elements) {
        super(ChannelStateSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ChannelStateSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ChannelStateSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ChannelStateSequence.create());
