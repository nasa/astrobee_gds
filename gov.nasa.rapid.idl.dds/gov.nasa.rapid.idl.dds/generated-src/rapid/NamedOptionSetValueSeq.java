...
/**
* A sequence of NamedOptionSetValue instances.
*/
public final class NamedOptionSetValueSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedOptionSetValueSeq() {
        super(NamedOptionSetValue.class);
    }

    public NamedOptionSetValueSeq (int initialMaximum) {
        super(NamedOptionSetValue.class, initialMaximum);
    }

    public NamedOptionSetValueSeq (Collection elements) {
        super(NamedOptionSetValue.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedOptionSetValue.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedOptionSetValue objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedOptionSetValue.create());
