...
/**
* A sequence of NamedOptionSetValueSequence16 instances.
*/
public final class NamedOptionSetValueSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedOptionSetValueSequence16Seq() {
        super(NamedOptionSetValueSequence16.class);
    }

    public NamedOptionSetValueSequence16Seq (int initialMaximum) {
        super(NamedOptionSetValueSequence16.class, initialMaximum);
    }

    public NamedOptionSetValueSequence16Seq (Collection elements) {
        super(NamedOptionSetValueSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedOptionSetValueSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedOptionSetValueSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedOptionSetValueSequence16.create());
