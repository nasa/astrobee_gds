...
/**
* A sequence of String32Sequence32 instances.
*/
public final class String32Sequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public String32Sequence32Seq() {
        super(String32Sequence32.class);
    }

    public String32Sequence32Seq (int initialMaximum) {
        super(String32Sequence32.class, initialMaximum);
    }

    public String32Sequence32Seq (Collection elements) {
        super(String32Sequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, String32Sequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' String32Sequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(String32Sequence32.create());
