...
/**
* A sequence of KeyTypeSequence16 instances.
*/
public final class KeyTypeSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeSequence16Seq() {
        super(KeyTypeSequence16.class);
    }

    public KeyTypeSequence16Seq (int initialMaximum) {
        super(KeyTypeSequence16.class, initialMaximum);
    }

    public KeyTypeSequence16Seq (Collection elements) {
        super(KeyTypeSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeSequence16.create());
