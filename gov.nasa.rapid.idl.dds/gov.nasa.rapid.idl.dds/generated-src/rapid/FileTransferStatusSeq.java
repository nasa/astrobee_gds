...
/**
* A sequence of FileTransferStatus instances.
*/
public final class FileTransferStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FileTransferStatusSeq() {
        super(FileTransferStatus.class);
    }

    public FileTransferStatusSeq (int initialMaximum) {
        super(FileTransferStatus.class, initialMaximum);
    }

    public FileTransferStatusSeq (Collection elements) {
        super(FileTransferStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FileTransferStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FileTransferStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FileTransferStatus.create());
