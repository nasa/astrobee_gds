...
/**
* A sequence of KeyTypeValueSequence8 instances.
*/
public final class KeyTypeValueSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeValueSequence8Seq() {
        super(KeyTypeValueSequence8.class);
    }

    public KeyTypeValueSequence8Seq (int initialMaximum) {
        super(KeyTypeValueSequence8.class, initialMaximum);
    }

    public KeyTypeValueSequence8Seq (Collection elements) {
        super(KeyTypeValueSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeValueSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeValueSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeValueSequence8.create());
