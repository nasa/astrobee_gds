...
/**
* A sequence of FrameDef instances.
*/
public final class FrameDefSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FrameDefSeq() {
        super(FrameDef.class);
    }

    public FrameDefSeq (int initialMaximum) {
        super(FrameDef.class, initialMaximum);
    }

    public FrameDefSeq (Collection elements) {
        super(FrameDef.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FrameDef.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FrameDef objects (beyond the original size of this object)
