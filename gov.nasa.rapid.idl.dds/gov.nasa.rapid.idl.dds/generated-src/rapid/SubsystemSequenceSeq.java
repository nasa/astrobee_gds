...
/**
* A sequence of SubsystemSequence instances.
*/
public final class SubsystemSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubsystemSequenceSeq() {
        super(SubsystemSequence.class);
    }

    public SubsystemSequenceSeq (int initialMaximum) {
        super(SubsystemSequence.class, initialMaximum);
    }

    public SubsystemSequenceSeq (Collection elements) {
        super(SubsystemSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubsystemSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubsystemSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubsystemSequence.create());
