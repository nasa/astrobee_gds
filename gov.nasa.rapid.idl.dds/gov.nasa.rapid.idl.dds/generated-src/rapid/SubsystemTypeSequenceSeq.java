...
/**
* A sequence of SubsystemTypeSequence instances.
*/
public final class SubsystemTypeSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SubsystemTypeSequenceSeq() {
        super(SubsystemTypeSequence.class);
    }

    public SubsystemTypeSequenceSeq (int initialMaximum) {
        super(SubsystemTypeSequence.class, initialMaximum);
    }

    public SubsystemTypeSequenceSeq (Collection elements) {
        super(SubsystemTypeSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SubsystemTypeSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SubsystemTypeSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SubsystemTypeSequence.create());
