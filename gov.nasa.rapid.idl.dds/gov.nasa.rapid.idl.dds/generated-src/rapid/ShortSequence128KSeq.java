...
/**
* A sequence of ShortSequence128K instances.
*/
public final class ShortSequence128KSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShortSequence128KSeq() {
        super(ShortSequence128K.class);
    }

    public ShortSequence128KSeq (int initialMaximum) {
        super(ShortSequence128K.class, initialMaximum);
    }

    public ShortSequence128KSeq (Collection elements) {
        super(ShortSequence128K.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShortSequence128K.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShortSequence128K objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShortSequence128K.create());
