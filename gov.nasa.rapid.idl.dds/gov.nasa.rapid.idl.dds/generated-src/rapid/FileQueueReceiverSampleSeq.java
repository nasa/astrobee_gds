...
/**
* A sequence of FileQueueReceiverSample instances.
*/
public final class FileQueueReceiverSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FileQueueReceiverSampleSeq() {
        super(FileQueueReceiverSample.class);
    }

    public FileQueueReceiverSampleSeq (int initialMaximum) {
        super(FileQueueReceiverSample.class, initialMaximum);
    }

    public FileQueueReceiverSampleSeq (Collection elements) {
        super(FileQueueReceiverSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FileQueueReceiverSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FileQueueReceiverSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FileQueueReceiverSample.create());
