...
/**
* A sequence of JointDef instances.
*/
public final class JointDefSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public JointDefSeq() {
        super(JointDef.class);
    }

    public JointDefSeq (int initialMaximum) {
        super(JointDef.class, initialMaximum);
    }

    public JointDefSeq (Collection elements) {
        super(JointDef.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, JointDef.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' JointDef objects (beyond the original size of this object)
