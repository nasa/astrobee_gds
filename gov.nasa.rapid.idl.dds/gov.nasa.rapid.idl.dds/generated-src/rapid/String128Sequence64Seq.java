...
/**
* A sequence of String128Sequence64 instances.
*/
public final class String128Sequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public String128Sequence64Seq() {
        super(String128Sequence64.class);
    }

    public String128Sequence64Seq (int initialMaximum) {
        super(String128Sequence64.class, initialMaximum);
    }

    public String128Sequence64Seq (Collection elements) {
        super(String128Sequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, String128Sequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' String128Sequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(String128Sequence64.create());
