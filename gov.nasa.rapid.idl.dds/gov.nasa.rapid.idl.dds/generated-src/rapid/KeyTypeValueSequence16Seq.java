...
/**
* A sequence of KeyTypeValueSequence16 instances.
*/
public final class KeyTypeValueSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeValueSequence16Seq() {
        super(KeyTypeValueSequence16.class);
    }

    public KeyTypeValueSequence16Seq (int initialMaximum) {
        super(KeyTypeValueSequence16.class, initialMaximum);
    }

    public KeyTypeValueSequence16Seq (Collection elements) {
        super(KeyTypeValueSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeValueSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeValueSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeValueSequence16.create());
