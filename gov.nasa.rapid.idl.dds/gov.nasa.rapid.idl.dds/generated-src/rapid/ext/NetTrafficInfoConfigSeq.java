...
/**
* A sequence of NetTrafficInfoConfig instances.
*/
public final class NetTrafficInfoConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NetTrafficInfoConfigSeq() {
        super(NetTrafficInfoConfig.class);
    }

    public NetTrafficInfoConfigSeq (int initialMaximum) {
        super(NetTrafficInfoConfig.class, initialMaximum);
    }

    public NetTrafficInfoConfigSeq (Collection elements) {
        super(NetTrafficInfoConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NetTrafficInfoConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NetTrafficInfoConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NetTrafficInfoConfig.create());
