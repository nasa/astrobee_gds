...
/**
* A sequence of OctetMapLayerSequence instances.
*/
public final class OctetMapLayerSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OctetMapLayerSequenceSeq() {
        super(OctetMapLayerSequence.class);
    }

    public OctetMapLayerSequenceSeq (int initialMaximum) {
        super(OctetMapLayerSequence.class, initialMaximum);
    }

    public OctetMapLayerSequenceSeq (Collection elements) {
        super(OctetMapLayerSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OctetMapLayerSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OctetMapLayerSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OctetMapLayerSequence.create());
