...
/**
* A sequence of ShortMapLayerSequence instances.
*/
public final class ShortMapLayerSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShortMapLayerSequenceSeq() {
        super(ShortMapLayerSequence.class);
    }

    public ShortMapLayerSequenceSeq (int initialMaximum) {
        super(ShortMapLayerSequence.class, initialMaximum);
    }

    public ShortMapLayerSequenceSeq (Collection elements) {
        super(ShortMapLayerSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShortMapLayerSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShortMapLayerSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShortMapLayerSequence.create());
