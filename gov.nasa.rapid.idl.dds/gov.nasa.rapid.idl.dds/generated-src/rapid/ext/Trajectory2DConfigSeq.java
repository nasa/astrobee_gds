...
/**
* A sequence of Trajectory2DConfig instances.
*/
public final class Trajectory2DConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public Trajectory2DConfigSeq() {
        super(Trajectory2DConfig.class);
    }

    public Trajectory2DConfigSeq (int initialMaximum) {
        super(Trajectory2DConfig.class, initialMaximum);
    }

    public Trajectory2DConfigSeq (Collection elements) {
        super(Trajectory2DConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Trajectory2DConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Trajectory2DConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(Trajectory2DConfig.create());
