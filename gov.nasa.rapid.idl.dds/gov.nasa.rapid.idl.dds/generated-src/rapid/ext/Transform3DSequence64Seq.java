...
/**
* A sequence of Transform3DSequence64 instances.
*/
public final class Transform3DSequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public Transform3DSequence64Seq() {
        super(Transform3DSequence64.class);
    }

    public Transform3DSequence64Seq (int initialMaximum) {
        super(Transform3DSequence64.class, initialMaximum);
    }

    public Transform3DSequence64Seq (Collection elements) {
        super(Transform3DSequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Transform3DSequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Transform3DSequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(Transform3DSequence64.create());
