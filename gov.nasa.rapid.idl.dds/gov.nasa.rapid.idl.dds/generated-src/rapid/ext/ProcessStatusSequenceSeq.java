...
/**
* A sequence of ProcessStatusSequence instances.
*/
public final class ProcessStatusSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ProcessStatusSequenceSeq() {
        super(ProcessStatusSequence.class);
    }

    public ProcessStatusSequenceSeq (int initialMaximum) {
        super(ProcessStatusSequence.class, initialMaximum);
    }

    public ProcessStatusSequenceSeq (Collection elements) {
        super(ProcessStatusSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ProcessStatusSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ProcessStatusSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ProcessStatusSequence.create());
