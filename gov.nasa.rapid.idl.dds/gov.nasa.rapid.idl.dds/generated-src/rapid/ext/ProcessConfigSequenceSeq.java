...
/**
* A sequence of ProcessConfigSequence instances.
*/
public final class ProcessConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ProcessConfigSequenceSeq() {
        super(ProcessConfigSequence.class);
    }

    public ProcessConfigSequenceSeq (int initialMaximum) {
        super(ProcessConfigSequence.class, initialMaximum);
    }

    public ProcessConfigSequenceSeq (Collection elements) {
        super(ProcessConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ProcessConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ProcessConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ProcessConfigSequence.create());
