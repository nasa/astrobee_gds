...
/**
* A sequence of ProcessManagerState instances.
*/
public final class ProcessManagerStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ProcessManagerStateSeq() {
        super(ProcessManagerState.class);
    }

    public ProcessManagerStateSeq (int initialMaximum) {
        super(ProcessManagerState.class, initialMaximum);
    }

    public ProcessManagerStateSeq (Collection elements) {
        super(ProcessManagerState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ProcessManagerState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ProcessManagerState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ProcessManagerState.create());
