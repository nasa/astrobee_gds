...
/**
* A sequence of FilesystemInfoSampleSequence instances.
*/
public final class FilesystemInfoSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FilesystemInfoSampleSequenceSeq() {
        super(FilesystemInfoSampleSequence.class);
    }

    public FilesystemInfoSampleSequenceSeq (int initialMaximum) {
        super(FilesystemInfoSampleSequence.class, initialMaximum);
    }

    public FilesystemInfoSampleSequenceSeq (Collection elements) {
        super(FilesystemInfoSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FilesystemInfoSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FilesystemInfoSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FilesystemInfoSampleSequence.create());
