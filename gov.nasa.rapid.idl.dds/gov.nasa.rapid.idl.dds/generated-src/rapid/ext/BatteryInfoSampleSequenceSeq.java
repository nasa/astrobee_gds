...
/**
* A sequence of BatteryInfoSampleSequence instances.
*/
public final class BatteryInfoSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryInfoSampleSequenceSeq() {
        super(BatteryInfoSampleSequence.class);
    }

    public BatteryInfoSampleSequenceSeq (int initialMaximum) {
        super(BatteryInfoSampleSequence.class, initialMaximum);
    }

    public BatteryInfoSampleSequenceSeq (Collection elements) {
        super(BatteryInfoSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryInfoSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryInfoSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryInfoSampleSequence.create());
