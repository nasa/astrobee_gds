...
/**
* A sequence of OctetMapLayerNameSequence instances.
*/
public final class OctetMapLayerNameSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OctetMapLayerNameSequenceSeq() {
        super(OctetMapLayerNameSequence.class);
    }

    public OctetMapLayerNameSequenceSeq (int initialMaximum) {
        super(OctetMapLayerNameSequence.class, initialMaximum);
    }

    public OctetMapLayerNameSequenceSeq (Collection elements) {
        super(OctetMapLayerNameSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OctetMapLayerNameSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OctetMapLayerNameSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OctetMapLayerNameSequence.create());
