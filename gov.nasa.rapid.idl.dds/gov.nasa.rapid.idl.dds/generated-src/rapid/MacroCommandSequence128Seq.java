...
/**
* A sequence of MacroCommandSequence128 instances.
*/
public final class MacroCommandSequence128Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MacroCommandSequence128Seq() {
        super(MacroCommandSequence128.class);
    }

    public MacroCommandSequence128Seq (int initialMaximum) {
        super(MacroCommandSequence128.class, initialMaximum);
    }

    public MacroCommandSequence128Seq (Collection elements) {
        super(MacroCommandSequence128.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MacroCommandSequence128.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MacroCommandSequence128 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MacroCommandSequence128.create());
