...
/**
* A sequence of ChannelConfigSequence instances.
*/
public final class ChannelConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ChannelConfigSequenceSeq() {
        super(ChannelConfigSequence.class);
    }

    public ChannelConfigSequenceSeq (int initialMaximum) {
        super(ChannelConfigSequence.class, initialMaximum);
    }

    public ChannelConfigSequenceSeq (Collection elements) {
        super(ChannelConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ChannelConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ChannelConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ChannelConfigSequence.create());
