...
/**
* A sequence of DoubleSequence512 instances.
*/
public final class DoubleSequence512Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DoubleSequence512Seq() {
        super(DoubleSequence512.class);
    }

    public DoubleSequence512Seq (int initialMaximum) {
        super(DoubleSequence512.class, initialMaximum);
    }

    public DoubleSequence512Seq (Collection elements) {
        super(DoubleSequence512.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DoubleSequence512.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DoubleSequence512 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DoubleSequence512.create());
