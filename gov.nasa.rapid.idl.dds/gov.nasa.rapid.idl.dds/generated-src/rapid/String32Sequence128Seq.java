...
/**
* A sequence of String32Sequence128 instances.
*/
public final class String32Sequence128Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public String32Sequence128Seq() {
        super(String32Sequence128.class);
    }

    public String32Sequence128Seq (int initialMaximum) {
        super(String32Sequence128.class, initialMaximum);
    }

    public String32Sequence128Seq (Collection elements) {
        super(String32Sequence128.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, String32Sequence128.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' String32Sequence128 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(String32Sequence128.create());
