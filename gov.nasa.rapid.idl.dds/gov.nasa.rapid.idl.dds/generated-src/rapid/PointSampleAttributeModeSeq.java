...
/**
* A sequence of PointSampleAttributeMode instances.
*/
public final class PointSampleAttributeModeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PointSampleAttributeModeSeq() {
        super(PointSampleAttributeMode.class);
    }

    public PointSampleAttributeModeSeq (int initialMaximum) {
        super(PointSampleAttributeMode.class, initialMaximum);
    }

    public PointSampleAttributeModeSeq (Collection elements) {
        super(PointSampleAttributeMode.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PointSampleAttributeMode.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PointSampleAttributeMode objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PointSampleAttributeMode.create());
