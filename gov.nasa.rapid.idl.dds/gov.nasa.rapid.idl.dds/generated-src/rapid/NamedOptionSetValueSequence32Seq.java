...
/**
* A sequence of NamedOptionSetValueSequence32 instances.
*/
public final class NamedOptionSetValueSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedOptionSetValueSequence32Seq() {
        super(NamedOptionSetValueSequence32.class);
    }

    public NamedOptionSetValueSequence32Seq (int initialMaximum) {
        super(NamedOptionSetValueSequence32.class, initialMaximum);
    }

    public NamedOptionSetValueSequence32Seq (Collection elements) {
        super(NamedOptionSetValueSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedOptionSetValueSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedOptionSetValueSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedOptionSetValueSequence32.create());
