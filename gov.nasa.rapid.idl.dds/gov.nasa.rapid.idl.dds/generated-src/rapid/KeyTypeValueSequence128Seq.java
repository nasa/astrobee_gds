...
/**
* A sequence of KeyTypeValueSequence128 instances.
*/
public final class KeyTypeValueSequence128Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeValueSequence128Seq() {
        super(KeyTypeValueSequence128.class);
    }

    public KeyTypeValueSequence128Seq (int initialMaximum) {
        super(KeyTypeValueSequence128.class, initialMaximum);
    }

    public KeyTypeValueSequence128Seq (Collection elements) {
        super(KeyTypeValueSequence128.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeValueSequence128.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeValueSequence128 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeValueSequence128.create());
