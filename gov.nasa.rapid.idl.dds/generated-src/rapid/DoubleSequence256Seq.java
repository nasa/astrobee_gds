...
/**
* A sequence of DoubleSequence256 instances.
*/
public final class DoubleSequence256Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DoubleSequence256Seq() {
        super(DoubleSequence256.class);
    }

    public DoubleSequence256Seq (int initialMaximum) {
        super(DoubleSequence256.class, initialMaximum);
    }

    public DoubleSequence256Seq (Collection elements) {
        super(DoubleSequence256.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DoubleSequence256.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DoubleSequence256 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DoubleSequence256.create());
