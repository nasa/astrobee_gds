...
/**
* A sequence of ParameterSequence16 instances.
*/
public final class ParameterSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ParameterSequence16Seq() {
        super(ParameterSequence16.class);
    }

    public ParameterSequence16Seq (int initialMaximum) {
        super(ParameterSequence16.class, initialMaximum);
    }

    public ParameterSequence16Seq (Collection elements) {
        super(ParameterSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ParameterSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ParameterSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ParameterSequence16.create());
