...
/**
* A sequence of AckCompletedStatus instances.
*/
public final class AckCompletedStatusSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AckCompletedStatusSeq() {
        super(AckCompletedStatus.class);
    }

    public AckCompletedStatusSeq (int initialMaximum) {
        super(AckCompletedStatus.class, initialMaximum);
    }

    public AckCompletedStatusSeq (Collection elements) {
        super(AckCompletedStatus.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AckCompletedStatus.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AckCompletedStatus objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AckCompletedStatus.create());
