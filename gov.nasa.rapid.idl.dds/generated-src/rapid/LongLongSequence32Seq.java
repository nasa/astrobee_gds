...
/**
* A sequence of LongLongSequence32 instances.
*/
public final class LongLongSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LongLongSequence32Seq() {
        super(LongLongSequence32.class);
    }

    public LongLongSequence32Seq (int initialMaximum) {
        super(LongLongSequence32.class, initialMaximum);
    }

    public LongLongSequence32Seq (Collection elements) {
        super(LongLongSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LongLongSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LongLongSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LongLongSequence32.create());
