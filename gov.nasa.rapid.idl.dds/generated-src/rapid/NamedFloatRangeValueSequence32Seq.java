...
/**
* A sequence of NamedFloatRangeValueSequence32 instances.
*/
public final class NamedFloatRangeValueSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedFloatRangeValueSequence32Seq() {
        super(NamedFloatRangeValueSequence32.class);
    }

    public NamedFloatRangeValueSequence32Seq (int initialMaximum) {
        super(NamedFloatRangeValueSequence32.class, initialMaximum);
    }

    public NamedFloatRangeValueSequence32Seq (Collection elements) {
        super(NamedFloatRangeValueSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedFloatRangeValueSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedFloatRangeValueSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedFloatRangeValueSequence32.create());
