...
/**
* A sequence of String256Sequence64 instances.
*/
public final class String256Sequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public String256Sequence64Seq() {
        super(String256Sequence64.class);
    }

    public String256Sequence64Seq (int initialMaximum) {
        super(String256Sequence64.class, initialMaximum);
    }

    public String256Sequence64Seq (Collection elements) {
        super(String256Sequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, String256Sequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' String256Sequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(String256Sequence64.create());
