...
/**
* A sequence of PointSampleSequence instances.
*/
public final class PointSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PointSampleSequenceSeq() {
        super(PointSampleSequence.class);
    }

    public PointSampleSequenceSeq (int initialMaximum) {
        super(PointSampleSequence.class, initialMaximum);
    }

    public PointSampleSequenceSeq (Collection elements) {
        super(PointSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PointSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PointSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PointSampleSequence.create());
