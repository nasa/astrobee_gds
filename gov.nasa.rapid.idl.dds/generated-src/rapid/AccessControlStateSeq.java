...
/**
* A sequence of AccessControlState instances.
*/
public final class AccessControlStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public AccessControlStateSeq() {
        super(AccessControlState.class);
    }

    public AccessControlStateSeq (int initialMaximum) {
        super(AccessControlState.class, initialMaximum);
    }

    public AccessControlStateSeq (Collection elements) {
        super(AccessControlState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, AccessControlState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' AccessControlState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(AccessControlState.create());
