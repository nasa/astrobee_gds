...
/**
* A sequence of MacroInfoSequence instances.
*/
public final class MacroInfoSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MacroInfoSequenceSeq() {
        super(MacroInfoSequence.class);
    }

    public MacroInfoSequenceSeq (int initialMaximum) {
        super(MacroInfoSequence.class, initialMaximum);
    }

    public MacroInfoSequenceSeq (Collection elements) {
        super(MacroInfoSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MacroInfoSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MacroInfoSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MacroInfoSequence.create());
