...
/**
* A sequence of String16Sequence16 instances.
*/
public final class String16Sequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public String16Sequence16Seq() {
        super(String16Sequence16.class);
    }

    public String16Sequence16Seq (int initialMaximum) {
        super(String16Sequence16.class, initialMaximum);
    }

    public String16Sequence16Seq (Collection elements) {
        super(String16Sequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, String16Sequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' String16Sequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(String16Sequence16.create());
