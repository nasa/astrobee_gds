...
/**
* A sequence of NamedFloatSequence instances.
*/
public final class NamedFloatSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedFloatSequenceSeq() {
        super(NamedFloatSequence.class);
    }

    public NamedFloatSequenceSeq (int initialMaximum) {
        super(NamedFloatSequence.class, initialMaximum);
    }

    public NamedFloatSequenceSeq (Collection elements) {
        super(NamedFloatSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedFloatSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedFloatSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedFloatSequence.create());
