...
/**
* A sequence of CommandDefSequence instances.
*/
public final class CommandDefSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CommandDefSequenceSeq() {
        super(CommandDefSequence.class);
    }

    public CommandDefSequenceSeq (int initialMaximum) {
        super(CommandDefSequence.class, initialMaximum);
    }

    public CommandDefSequenceSeq (Collection elements) {
        super(CommandDefSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CommandDefSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CommandDefSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CommandDefSequence.create());
