...
/**
* A sequence of NamedFloatRangeValueSequence16 instances.
*/
public final class NamedFloatRangeValueSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NamedFloatRangeValueSequence16Seq() {
        super(NamedFloatRangeValueSequence16.class);
    }

    public NamedFloatRangeValueSequence16Seq (int initialMaximum) {
        super(NamedFloatRangeValueSequence16.class, initialMaximum);
    }

    public NamedFloatRangeValueSequence16Seq (Collection elements) {
        super(NamedFloatRangeValueSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NamedFloatRangeValueSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NamedFloatRangeValueSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NamedFloatRangeValueSequence16.create());
