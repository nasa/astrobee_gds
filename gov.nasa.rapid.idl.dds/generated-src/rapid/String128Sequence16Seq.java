...
/**
* A sequence of String128Sequence16 instances.
*/
public final class String128Sequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public String128Sequence16Seq() {
        super(String128Sequence16.class);
    }

    public String128Sequence16Seq (int initialMaximum) {
        super(String128Sequence16.class, initialMaximum);
    }

    public String128Sequence16Seq (Collection elements) {
        super(String128Sequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, String128Sequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' String128Sequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(String128Sequence16.create());
