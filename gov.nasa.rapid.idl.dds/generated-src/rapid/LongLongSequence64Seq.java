...
/**
* A sequence of LongLongSequence64 instances.
*/
public final class LongLongSequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public LongLongSequence64Seq() {
        super(LongLongSequence64.class);
    }

    public LongLongSequence64Seq (int initialMaximum) {
        super(LongLongSequence64.class, initialMaximum);
    }

    public LongLongSequence64Seq (Collection elements) {
        super(LongLongSequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, LongLongSequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' LongLongSequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LongLongSequence64.create());
