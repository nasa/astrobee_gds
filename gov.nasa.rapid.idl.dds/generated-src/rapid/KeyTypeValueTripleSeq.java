...
/**
* A sequence of KeyTypeValueTriple instances.
*/
public final class KeyTypeValueTripleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeValueTripleSeq() {
        super(KeyTypeValueTriple.class);
    }

    public KeyTypeValueTripleSeq (int initialMaximum) {
        super(KeyTypeValueTriple.class, initialMaximum);
    }

    public KeyTypeValueTripleSeq (Collection elements) {
        super(KeyTypeValueTriple.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeValueTriple.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeValueTriple objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeValueTriple.create());
