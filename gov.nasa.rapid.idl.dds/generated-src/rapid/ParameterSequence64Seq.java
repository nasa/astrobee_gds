...
/**
* A sequence of ParameterSequence64 instances.
*/
public final class ParameterSequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ParameterSequence64Seq() {
        super(ParameterSequence64.class);
    }

    public ParameterSequence64Seq (int initialMaximum) {
        super(ParameterSequence64.class, initialMaximum);
    }

    public ParameterSequence64Seq (Collection elements) {
        super(ParameterSequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ParameterSequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ParameterSequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ParameterSequence64.create());
