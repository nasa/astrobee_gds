...
/**
* A sequence of OctetSequence128K instances.
*/
public final class OctetSequence128KSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public OctetSequence128KSeq() {
        super(OctetSequence128K.class);
    }

    public OctetSequence128KSeq (int initialMaximum) {
        super(OctetSequence128K.class, initialMaximum);
    }

    public OctetSequence128KSeq (Collection elements) {
        super(OctetSequence128K.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, OctetSequence128K.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' OctetSequence128K objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(OctetSequence128K.create());
