...
/**
* A sequence of BatteryInfoConfig instances.
*/
public final class BatteryInfoConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryInfoConfigSeq() {
        super(BatteryInfoConfig.class);
    }

    public BatteryInfoConfigSeq (int initialMaximum) {
        super(BatteryInfoConfig.class, initialMaximum);
    }

    public BatteryInfoConfigSeq (Collection elements) {
        super(BatteryInfoConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryInfoConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryInfoConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryInfoConfig.create());
