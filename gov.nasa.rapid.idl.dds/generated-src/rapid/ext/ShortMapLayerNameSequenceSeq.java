...
/**
* A sequence of ShortMapLayerNameSequence instances.
*/
public final class ShortMapLayerNameSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ShortMapLayerNameSequenceSeq() {
        super(ShortMapLayerNameSequence.class);
    }

    public ShortMapLayerNameSequenceSeq (int initialMaximum) {
        super(ShortMapLayerNameSequence.class, initialMaximum);
    }

    public ShortMapLayerNameSequenceSeq (Collection elements) {
        super(ShortMapLayerNameSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ShortMapLayerNameSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ShortMapLayerNameSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ShortMapLayerNameSequence.create());
