...
/**
* A sequence of WifiInfoSampleSequence instances.
*/
public final class WifiInfoSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WifiInfoSampleSequenceSeq() {
        super(WifiInfoSampleSequence.class);
    }

    public WifiInfoSampleSequenceSeq (int initialMaximum) {
        super(WifiInfoSampleSequence.class, initialMaximum);
    }

    public WifiInfoSampleSequenceSeq (Collection elements) {
        super(WifiInfoSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WifiInfoSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WifiInfoSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(WifiInfoSampleSequence.create());
