...
/**
* A sequence of RTrans2DInterpretation instances.
*/
public final class RTrans2DInterpretationSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RTrans2DInterpretationSeq() {
        super(RTrans2DInterpretation.class);
    }

    public RTrans2DInterpretationSeq (int initialMaximum) {
        super(RTrans2DInterpretation.class, initialMaximum);
    }

    public RTrans2DInterpretationSeq (Collection elements) {
        super(RTrans2DInterpretation.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RTrans2DInterpretation.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RTrans2DInterpretation objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RTrans2DInterpretation.create());
