...
/**
* A sequence of ThermalInfoConfigSequence instances.
*/
public final class ThermalInfoConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThermalInfoConfigSequenceSeq() {
        super(ThermalInfoConfigSequence.class);
    }

    public ThermalInfoConfigSequenceSeq (int initialMaximum) {
        super(ThermalInfoConfigSequence.class, initialMaximum);
    }

    public ThermalInfoConfigSequenceSeq (Collection elements) {
        super(ThermalInfoConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThermalInfoConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThermalInfoConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThermalInfoConfigSequence.create());
