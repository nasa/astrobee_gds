...
/**
* A sequence of ThermalInfoSample instances.
*/
public final class ThermalInfoSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThermalInfoSampleSeq() {
        super(ThermalInfoSample.class);
    }

    public ThermalInfoSampleSeq (int initialMaximum) {
        super(ThermalInfoSample.class, initialMaximum);
    }

    public ThermalInfoSampleSeq (Collection elements) {
        super(ThermalInfoSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThermalInfoSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThermalInfoSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThermalInfoSample.create());
