...
/**
* A sequence of BatteryInfoSample instances.
*/
public final class BatteryInfoSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryInfoSampleSeq() {
        super(BatteryInfoSample.class);
    }

    public BatteryInfoSampleSeq (int initialMaximum) {
        super(BatteryInfoSample.class, initialMaximum);
    }

    public BatteryInfoSampleSeq (Collection elements) {
        super(BatteryInfoSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryInfoSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryInfoSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryInfoSample.create());
