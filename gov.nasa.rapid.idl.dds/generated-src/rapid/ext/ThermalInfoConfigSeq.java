...
/**
* A sequence of ThermalInfoConfig instances.
*/
public final class ThermalInfoConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThermalInfoConfigSeq() {
        super(ThermalInfoConfig.class);
    }

    public ThermalInfoConfigSeq (int initialMaximum) {
        super(ThermalInfoConfig.class, initialMaximum);
    }

    public ThermalInfoConfigSeq (Collection elements) {
        super(ThermalInfoConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThermalInfoConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThermalInfoConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThermalInfoConfig.create());
