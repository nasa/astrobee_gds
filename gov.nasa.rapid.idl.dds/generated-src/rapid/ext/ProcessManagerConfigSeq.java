...
/**
* A sequence of ProcessManagerConfig instances.
*/
public final class ProcessManagerConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ProcessManagerConfigSeq() {
        super(ProcessManagerConfig.class);
    }

    public ProcessManagerConfigSeq (int initialMaximum) {
        super(ProcessManagerConfig.class, initialMaximum);
    }

    public ProcessManagerConfigSeq (Collection elements) {
        super(ProcessManagerConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ProcessManagerConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ProcessManagerConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ProcessManagerConfig.create());
