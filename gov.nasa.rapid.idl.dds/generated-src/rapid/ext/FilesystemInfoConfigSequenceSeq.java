...
/**
* A sequence of FilesystemInfoConfigSequence instances.
*/
public final class FilesystemInfoConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FilesystemInfoConfigSequenceSeq() {
        super(FilesystemInfoConfigSequence.class);
    }

    public FilesystemInfoConfigSequenceSeq (int initialMaximum) {
        super(FilesystemInfoConfigSequence.class, initialMaximum);
    }

    public FilesystemInfoConfigSequenceSeq (Collection elements) {
        super(FilesystemInfoConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FilesystemInfoConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FilesystemInfoConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FilesystemInfoConfigSequence.create());
