...
/**
* A sequence of NetTrafficInfoSampleSequence instances.
*/
public final class NetTrafficInfoSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NetTrafficInfoSampleSequenceSeq() {
        super(NetTrafficInfoSampleSequence.class);
    }

    public NetTrafficInfoSampleSequenceSeq (int initialMaximum) {
        super(NetTrafficInfoSampleSequence.class, initialMaximum);
    }

    public NetTrafficInfoSampleSequenceSeq (Collection elements) {
        super(NetTrafficInfoSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NetTrafficInfoSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NetTrafficInfoSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NetTrafficInfoSampleSequence.create());
