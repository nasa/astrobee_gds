...
/**
* A sequence of RangeScanDirection instances.
*/
public final class RangeScanDirectionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RangeScanDirectionSeq() {
        super(RangeScanDirection.class);
    }

    public RangeScanDirectionSeq (int initialMaximum) {
        super(RangeScanDirection.class, initialMaximum);
    }

    public RangeScanDirectionSeq (Collection elements) {
        super(RangeScanDirection.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RangeScanDirection.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RangeScanDirection objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RangeScanDirection.create());
