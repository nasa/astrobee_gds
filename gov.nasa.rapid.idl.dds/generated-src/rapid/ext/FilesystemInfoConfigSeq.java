...
/**
* A sequence of FilesystemInfoConfig instances.
*/
public final class FilesystemInfoConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FilesystemInfoConfigSeq() {
        super(FilesystemInfoConfig.class);
    }

    public FilesystemInfoConfigSeq (int initialMaximum) {
        super(FilesystemInfoConfig.class, initialMaximum);
    }

    public FilesystemInfoConfigSeq (Collection elements) {
        super(FilesystemInfoConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FilesystemInfoConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FilesystemInfoConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FilesystemInfoConfig.create());
