...
/**
* A sequence of NetTrafficInfoConfigSequence instances.
*/
public final class NetTrafficInfoConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NetTrafficInfoConfigSequenceSeq() {
        super(NetTrafficInfoConfigSequence.class);
    }

    public NetTrafficInfoConfigSequenceSeq (int initialMaximum) {
        super(NetTrafficInfoConfigSequence.class, initialMaximum);
    }

    public NetTrafficInfoConfigSequenceSeq (Collection elements) {
        super(NetTrafficInfoConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NetTrafficInfoConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NetTrafficInfoConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NetTrafficInfoConfigSequence.create());
