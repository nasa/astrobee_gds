...
/**
* A sequence of WifiInfoConfigSequence instances.
*/
public final class WifiInfoConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public WifiInfoConfigSequenceSeq() {
        super(WifiInfoConfigSequence.class);
    }

    public WifiInfoConfigSequenceSeq (int initialMaximum) {
        super(WifiInfoConfigSequence.class, initialMaximum);
    }

    public WifiInfoConfigSequenceSeq (Collection elements) {
        super(WifiInfoConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, WifiInfoConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' WifiInfoConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(WifiInfoConfigSequence.create());
