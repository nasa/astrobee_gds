...
/**
* A sequence of BatteryInfoConfigSequence instances.
*/
public final class BatteryInfoConfigSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryInfoConfigSequenceSeq() {
        super(BatteryInfoConfigSequence.class);
    }

    public BatteryInfoConfigSequenceSeq (int initialMaximum) {
        super(BatteryInfoConfigSequence.class, initialMaximum);
    }

    public BatteryInfoConfigSequenceSeq (Collection elements) {
        super(BatteryInfoConfigSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryInfoConfigSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryInfoConfigSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryInfoConfigSequence.create());
