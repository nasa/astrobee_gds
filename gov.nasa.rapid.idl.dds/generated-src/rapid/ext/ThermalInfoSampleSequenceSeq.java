...
/**
* A sequence of ThermalInfoSampleSequence instances.
*/
public final class ThermalInfoSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ThermalInfoSampleSequenceSeq() {
        super(ThermalInfoSampleSequence.class);
    }

    public ThermalInfoSampleSequenceSeq (int initialMaximum) {
        super(ThermalInfoSampleSequence.class, initialMaximum);
    }

    public ThermalInfoSampleSequenceSeq (Collection elements) {
        super(ThermalInfoSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ThermalInfoSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ThermalInfoSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ThermalInfoSampleSequence.create());
