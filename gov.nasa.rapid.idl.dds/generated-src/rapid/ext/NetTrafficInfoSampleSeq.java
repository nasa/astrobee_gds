...
/**
* A sequence of NetTrafficInfoSample instances.
*/
public final class NetTrafficInfoSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public NetTrafficInfoSampleSeq() {
        super(NetTrafficInfoSample.class);
    }

    public NetTrafficInfoSampleSeq (int initialMaximum) {
        super(NetTrafficInfoSample.class, initialMaximum);
    }

    public NetTrafficInfoSampleSeq (Collection elements) {
        super(NetTrafficInfoSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, NetTrafficInfoSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' NetTrafficInfoSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(NetTrafficInfoSample.create());
