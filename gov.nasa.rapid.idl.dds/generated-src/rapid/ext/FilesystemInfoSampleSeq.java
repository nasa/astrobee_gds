...
/**
* A sequence of FilesystemInfoSample instances.
*/
public final class FilesystemInfoSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FilesystemInfoSampleSeq() {
        super(FilesystemInfoSample.class);
    }

    public FilesystemInfoSampleSeq (int initialMaximum) {
        super(FilesystemInfoSample.class, initialMaximum);
    }

    public FilesystemInfoSampleSeq (Collection elements) {
        super(FilesystemInfoSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FilesystemInfoSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FilesystemInfoSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FilesystemInfoSample.create());
