...
/**
* A sequence of Trajectory2DSample instances.
*/
public final class Trajectory2DSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public Trajectory2DSampleSeq() {
        super(Trajectory2DSample.class);
    }

    public Trajectory2DSampleSeq (int initialMaximum) {
        super(Trajectory2DSample.class, initialMaximum);
    }

    public Trajectory2DSampleSeq (Collection elements) {
        super(Trajectory2DSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, Trajectory2DSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' Trajectory2DSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(Trajectory2DSample.create());
