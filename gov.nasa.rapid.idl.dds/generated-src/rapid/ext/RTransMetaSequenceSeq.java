...
/**
* A sequence of RTransMetaSequence instances.
*/
public final class RTransMetaSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RTransMetaSequenceSeq() {
        super(RTransMetaSequence.class);
    }

    public RTransMetaSequenceSeq (int initialMaximum) {
        super(RTransMetaSequence.class, initialMaximum);
    }

    public RTransMetaSequenceSeq (Collection elements) {
        super(RTransMetaSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RTransMetaSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RTransMetaSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(RTransMetaSequence.create());
