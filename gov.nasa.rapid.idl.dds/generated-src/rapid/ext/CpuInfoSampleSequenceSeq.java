...
/**
* A sequence of CpuInfoSampleSequence instances.
*/
public final class CpuInfoSampleSequenceSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CpuInfoSampleSequenceSeq() {
        super(CpuInfoSampleSequence.class);
    }

    public CpuInfoSampleSequenceSeq (int initialMaximum) {
        super(CpuInfoSampleSequence.class, initialMaximum);
    }

    public CpuInfoSampleSequenceSeq (Collection elements) {
        super(CpuInfoSampleSequence.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CpuInfoSampleSequence.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CpuInfoSampleSequence objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CpuInfoSampleSequence.create());
