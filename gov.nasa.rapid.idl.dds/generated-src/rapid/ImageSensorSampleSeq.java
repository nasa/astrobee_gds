...
/**
* A sequence of ImageSensorSample instances.
*/
public final class ImageSensorSampleSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ImageSensorSampleSeq() {
        super(ImageSensorSample.class);
    }

    public ImageSensorSampleSeq (int initialMaximum) {
        super(ImageSensorSample.class, initialMaximum);
    }

    public ImageSensorSampleSeq (Collection elements) {
        super(ImageSensorSample.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ImageSensorSample.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ImageSensorSample objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ImageSensorSample.create());
