...
/**
* A sequence of KeyTypeValueSequence32 instances.
*/
public final class KeyTypeValueSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeValueSequence32Seq() {
        super(KeyTypeValueSequence32.class);
    }

    public KeyTypeValueSequence32Seq (int initialMaximum) {
        super(KeyTypeValueSequence32.class, initialMaximum);
    }

    public KeyTypeValueSequence32Seq (Collection elements) {
        super(KeyTypeValueSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeValueSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeValueSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeValueSequence32.create());
