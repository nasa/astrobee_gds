...
/**
* A sequence of DoubleSequence128 instances.
*/
public final class DoubleSequence128Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DoubleSequence128Seq() {
        super(DoubleSequence128.class);
    }

    public DoubleSequence128Seq (int initialMaximum) {
        super(DoubleSequence128.class, initialMaximum);
    }

    public DoubleSequence128Seq (Collection elements) {
        super(DoubleSequence128.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DoubleSequence128.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DoubleSequence128 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DoubleSequence128.create());
