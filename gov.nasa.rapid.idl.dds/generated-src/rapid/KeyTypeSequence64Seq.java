...
/**
* A sequence of KeyTypeSequence64 instances.
*/
public final class KeyTypeSequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public KeyTypeSequence64Seq() {
        super(KeyTypeSequence64.class);
    }

    public KeyTypeSequence64Seq (int initialMaximum) {
        super(KeyTypeSequence64.class, initialMaximum);
    }

    public KeyTypeSequence64Seq (Collection elements) {
        super(KeyTypeSequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, KeyTypeSequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' KeyTypeSequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(KeyTypeSequence64.create());
