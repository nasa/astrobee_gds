...
/**
* A sequence of PointSampleXyzMode instances.
*/
public final class PointSampleXyzModeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PointSampleXyzModeSeq() {
        super(PointSampleXyzMode.class);
    }

    public PointSampleXyzModeSeq (int initialMaximum) {
        super(PointSampleXyzMode.class, initialMaximum);
    }

    public PointSampleXyzModeSeq (Collection elements) {
        super(PointSampleXyzMode.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PointSampleXyzMode.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PointSampleXyzMode objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(PointSampleXyzMode.create());
