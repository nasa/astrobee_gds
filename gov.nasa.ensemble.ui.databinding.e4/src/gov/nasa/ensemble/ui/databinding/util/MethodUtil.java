...
public class MethodUtil {
    ...
    public static Method getSetMethod(Class pClass, String property, Class setType) throws SecurityException, NoSuchMethodException{
        ...
        return pClass.getMethod(SET + StringUtil.upperFirstChar(property, false), setType);
    }
    ...
    public static Method getGetMethod(Class pClass, String property) throws SecurityException, NoSuchMethodException{
        ...
        return pClass.getMethod(GET + StringUtil.upperFirstChar(property, false), (Class[])null);
    }
    ...
    public static Method getIsMethod(Class pClass, String property) throws SecurityException, NoSuchMethodException{
        ...
        return pClass.getMethod(IS + StringUtil.upperFirstChar(property, false), (Class[]) null);
    }
    ...
    /**
     * Get the method for this class
     * @param property
     * @return
     * @throws SecurityException
     * @throws NoSuchMethodException
     */
    public static Method getMethod(Class pClass, String method) throws SecurityException, NoSuchMethodException{
        if (pClass == null){
            throw new NoSuchMethodException("class is null");
        }
        return pClass.getMethod(method, (Class[])null);
    }
    ...
    public static boolean isTriggerMethod(Method method){
    	return method.getName().startsWith(TRIGGER);
    }
    
    public static String getTriggerSuffix(String methodName){
    	if (methodName == null || methodName.isEmpty()){
    		return "";
    	}
    	if (methodName.startsWith(TRIGGER)){
    		return methodName.substring(TRIGGER.length());
    	}
    	return methodName;
    }
    ...
    /**
     * Check if the methods of this class contain at least one getter and setter pair.
     * @param pClass
     * @return
     */
    public static boolean hasGetterSetter(Class pClass){
    	for (Method m : pClass.getMethods()) {
    		if (isGetMethod(m) || isIsMethod(m)){
    			String suffix = getSuffix(m);
    			try {
					Method setMethod = getSetMethod(pClass, suffix, m.getReturnType());
					if (setMethod != null){
						return true;
					}
