...
/**
* A sequence of CameraInfoSequence8 instances.
*/
public final class CameraInfoSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CameraInfoSequence8Seq() {
        super(CameraInfoSequence8.class);
    }

    public CameraInfoSequence8Seq (int initialMaximum) {
        super(CameraInfoSequence8.class, initialMaximum);
    }

    public CameraInfoSequence8Seq (Collection elements) {
        super(CameraInfoSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CameraInfoSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CameraInfoSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CameraInfoSequence8.create());
