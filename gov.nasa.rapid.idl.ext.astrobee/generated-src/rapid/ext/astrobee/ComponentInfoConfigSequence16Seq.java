...
/**
* A sequence of ComponentInfoConfigSequence16 instances.
*/
public final class ComponentInfoConfigSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ComponentInfoConfigSequence16Seq() {
        super(ComponentInfoConfigSequence16.class);
    }

    public ComponentInfoConfigSequence16Seq (int initialMaximum) {
        super(ComponentInfoConfigSequence16.class, initialMaximum);
    }

    public ComponentInfoConfigSequence16Seq (Collection elements) {
        super(ComponentInfoConfigSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ComponentInfoConfigSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ComponentInfoConfigSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ComponentInfoConfigSequence16.create());
