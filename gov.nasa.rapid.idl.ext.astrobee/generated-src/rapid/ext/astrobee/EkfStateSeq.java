...
/**
* A sequence of EkfState instances.
*/
public final class EkfStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EkfStateSeq() {
        super(EkfState.class);
    }

    public EkfStateSeq (int initialMaximum) {
        super(EkfState.class, initialMaximum);
    }

    public EkfStateSeq (Collection elements) {
        super(EkfState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EkfState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EkfState objects (beyond the original size of this object)
