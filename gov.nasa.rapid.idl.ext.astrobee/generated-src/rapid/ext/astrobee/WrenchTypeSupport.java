...
public class WrenchTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "rapid::ext::astrobee::Wrench";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final WrenchTypeSupport _singleton
    = new WrenchTypeSupport();
    ...
    */
    public static WrenchTypeSupport get_instance() {
        return _singleton;
    }

    public static WrenchTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return WrenchTypeCode.VALUE;
    }

    public Object create_data() {
        return Wrench.create();
    }
    ...
    * <code>this</code> is not a <code>Wrench</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        Wrench typedDst = (Wrench) destination;
        Wrench typedSrc = (Wrench) source;
        ...
        Object sample) 
    {
        ...
        currentAlignment += rapid.Vec3dTypeSupport.get_instance().get_serialized_sample_size(
            endpoint_data,false,encapsulation_id,currentAlignment,typedSrc.force);
        currentAlignment += rapid.Vec3dTypeSupport.get_instance().get_serialized_sample_size(
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            rapid.Vec3dTypeSupport.get_instance().serialize(endpoint_data, typedSrc.force, dst, false, encapsulation_id,true,endpoint_plugin_qos);

            rapid.Vec3dTypeSupport.get_instance().serialize(endpoint_data, typedSrc.torque, dst, false, encapsulation_id,true,endpoint_plugin_qos);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            Wrench typedDst = (Wrench) dst;
            typedDst.clear();      
            try{
                typedDst.force = (rapid.Vec3d)rapid.Vec3dTypeSupport.get_instance().deserialize_sample(endpoint_data, typedDst.force, src, false, true, endpoint_plugin_qos);     
                typedDst.torque = (rapid.Vec3d)rapid.Vec3dTypeSupport.get_instance().deserialize_sample(endpoint_data, typedDst.torque, src, false, true, endpoint_plugin_qos);     
    ...
    int mask) {
        return new WrenchDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new WrenchDataReader(native_reader, listener, mask, this);   
    ...
    protected WrenchTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME, false,WrenchTypeCode.VALUE,Wrench.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected WrenchTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,WrenchTypeCode.VALUE,Wrench.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
