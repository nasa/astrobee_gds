...
/**
* A sequence of GuestScienceCommandSequence32 instances.
*/
public final class GuestScienceCommandSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GuestScienceCommandSequence32Seq() {
        super(GuestScienceCommandSequence32.class);
    }

    public GuestScienceCommandSequence32Seq (int initialMaximum) {
        super(GuestScienceCommandSequence32.class, initialMaximum);
    }

    public GuestScienceCommandSequence32Seq (Collection elements) {
        super(GuestScienceCommandSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GuestScienceCommandSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GuestScienceCommandSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GuestScienceCommandSequence32.create());
