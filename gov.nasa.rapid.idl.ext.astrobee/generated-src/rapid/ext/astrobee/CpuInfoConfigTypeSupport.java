...
public class CpuInfoConfigTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "rapid::ext::astrobee::CpuInfoConfig";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final CpuInfoConfigTypeSupport _singleton
    = new CpuInfoConfigTypeSupport();
    ...
    */
    public static CpuInfoConfigTypeSupport get_instance() {
        return _singleton;
    }

    public static CpuInfoConfigTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return CpuInfoConfigTypeCode.VALUE;
    }

    public Object create_data() {
        return CpuInfoConfig.create();
    }
    ...
    * <code>this</code> is not a <code>CpuInfoConfig</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        CpuInfoConfig typedDst = (CpuInfoConfig) destination;
        CpuInfoConfig typedSrc = (CpuInfoConfig) source;
    ...
    public long get_serialized_sample_max_size(Object endpoint_data,boolean include_encapsulation,short encapsulation_id,long currentAlignment) {
        ...
        currentAlignment += rapid.ext.astrobee.UnsignedLongSequence8TypeSupport.get_instance().get_serialized_sample_max_size(endpoint_data,false, encapsulation_id,currentAlignment);
        if (include_encapsulation) {
    ...
    public long get_serialized_sample_min_size(Object endpoint_data,boolean include_encapsulation,short encapsulation_id,long currentAlignment) {
        ...
        currentAlignment += CdrPrimitiveType.SHORT.getMaxSizeSerialized(currentAlignment );
        currentAlignment += rapid.ext.astrobee.UnsignedLongSequence8TypeSupport.get_instance().get_serialized_sample_min_size(endpoint_data,false, encapsulation_id,currentAlignment);
    ...
    public long get_serialized_sample_size(
        ...
        Object sample) 
    {
        CpuInfoConfig typedSrc = (CpuInfoConfig) sample;
        long origAlignment = currentAlignment;
        long encapsulation_size = currentAlignment;
        ...
        currentAlignment  +=  CdrPrimitiveType.SHORT.getMaxSizeSerialized(currentAlignment);
        currentAlignment += rapid.ext.astrobee.UnsignedLongSequence8TypeSupport.get_instance().get_serialized_sample_size(
            endpoint_data,false,encapsulation_id,currentAlignment,typedSrc.max_frequencies);
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            dst.writeString(typedSrc.name,16);

            dst.writeShort(typedSrc.num_cpus);

            rapid.ext.astrobee.UnsignedLongSequence8TypeSupport.get_instance().serialize(endpoint_data, typedSrc.max_frequencies, dst, false, encapsulation_id,true,endpoint_plugin_qos);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            CpuInfoConfig typedDst = (CpuInfoConfig) dst;
            typedDst.clear();      
            try{
                typedDst.name = src.readString(16);
                typedDst.num_cpus = src.readShort();
                typedDst.max_frequencies = (rapid.ext.astrobee.UnsignedLongSequence8)rapid.ext.astrobee.UnsignedLongSequence8TypeSupport.get_instance().deserialize_sample(endpoint_data, typedDst.max_frequencies, src, false, true, endpoint_plugin_qos);     
    ...
    Object endpoint_plugin_qos)
    {
        ...
        if (skip_sample) {
            ...
            rapid.ext.astrobee.UnsignedLongSequence8TypeSupport.get_instance().skip(endpoint_data, src, false, true, endpoint_plugin_qos);
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if (deserialize_key) {
            ...
            CpuInfoConfig typedDst = (CpuInfoConfig) sample;
    ...
    int mask) {
        return new CpuInfoConfigDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new CpuInfoConfigDataReader(native_reader, listener, mask, this);   
    ...
    protected CpuInfoConfigTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME, false,CpuInfoConfigTypeCode.VALUE,CpuInfoConfig.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected CpuInfoConfigTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,CpuInfoConfigTypeCode.VALUE,CpuInfoConfig.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
