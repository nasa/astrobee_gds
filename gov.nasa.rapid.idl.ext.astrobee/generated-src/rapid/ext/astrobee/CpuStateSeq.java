...
/**
* A sequence of CpuState instances.
*/
public final class CpuStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CpuStateSeq() {
        super(CpuState.class);
    }

    public CpuStateSeq (int initialMaximum) {
        super(CpuState.class, initialMaximum);
    }

    public CpuStateSeq (Collection elements) {
        super(CpuState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CpuState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CpuState objects (beyond the original size of this object)
