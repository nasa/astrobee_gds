...
/**
* A sequence of DiskInfoSequence8 instances.
*/
public final class DiskInfoSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiskInfoSequence8Seq() {
        super(DiskInfoSequence8.class);
    }

    public DiskInfoSequence8Seq (int initialMaximum) {
        super(DiskInfoSequence8.class, initialMaximum);
    }

    public DiskInfoSequence8Seq (Collection elements) {
        super(DiskInfoSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiskInfoSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiskInfoSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiskInfoSequence8.create());
