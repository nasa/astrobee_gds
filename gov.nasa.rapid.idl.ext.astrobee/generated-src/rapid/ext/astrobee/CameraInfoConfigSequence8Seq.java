...
/**
* A sequence of CameraInfoConfigSequence8 instances.
*/
public final class CameraInfoConfigSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CameraInfoConfigSequence8Seq() {
        super(CameraInfoConfigSequence8.class);
    }

    public CameraInfoConfigSequence8Seq (int initialMaximum) {
        super(CameraInfoConfigSequence8.class, initialMaximum);
    }

    public CameraInfoConfigSequence8Seq (Collection elements) {
        super(CameraInfoConfigSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CameraInfoConfigSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CameraInfoConfigSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CameraInfoConfigSequence8.create());
