...
/**
* A sequence of CameraResolutionSequence8 instances.
*/
public final class CameraResolutionSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CameraResolutionSequence8Seq() {
        super(CameraResolutionSequence8.class);
    }

    public CameraResolutionSequence8Seq (int initialMaximum) {
        super(CameraResolutionSequence8.class, initialMaximum);
    }

    public CameraResolutionSequence8Seq (Collection elements) {
        super(CameraResolutionSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CameraResolutionSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CameraResolutionSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CameraResolutionSequence8.create());
