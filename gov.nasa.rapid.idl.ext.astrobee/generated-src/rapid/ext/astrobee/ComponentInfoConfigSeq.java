...
/**
* A sequence of ComponentInfoConfig instances.
*/
public final class ComponentInfoConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ComponentInfoConfigSeq() {
        super(ComponentInfoConfig.class);
    }

    public ComponentInfoConfigSeq (int initialMaximum) {
        super(ComponentInfoConfig.class, initialMaximum);
    }

    public ComponentInfoConfigSeq (Collection elements) {
        super(ComponentInfoConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ComponentInfoConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ComponentInfoConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ComponentInfoConfig.create());
