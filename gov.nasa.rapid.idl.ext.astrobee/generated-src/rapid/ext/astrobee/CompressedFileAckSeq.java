...
/**
* A sequence of CompressedFileAck instances.
*/
public final class CompressedFileAckSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CompressedFileAckSeq() {
        super(CompressedFileAck.class);
    }

    public CompressedFileAckSeq (int initialMaximum) {
        super(CompressedFileAck.class, initialMaximum);
    }

    public CompressedFileAckSeq (Collection elements) {
        super(CompressedFileAck.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CompressedFileAck.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CompressedFileAck objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CompressedFileAck.create());
