...
/**
* A sequence of BatteryInfoSequence4 instances.
*/
public final class BatteryInfoSequence4Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryInfoSequence4Seq() {
        super(BatteryInfoSequence4.class);
    }

    public BatteryInfoSequence4Seq (int initialMaximum) {
        super(BatteryInfoSequence4.class, initialMaximum);
    }

    public BatteryInfoSequence4Seq (Collection elements) {
        super(BatteryInfoSequence4.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryInfoSequence4.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryInfoSequence4 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryInfoSequence4.create());
