...
/**
* A sequence of ComponentInfoSequence16 instances.
*/
public final class ComponentInfoSequence16Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ComponentInfoSequence16Seq() {
        super(ComponentInfoSequence16.class);
    }

    public ComponentInfoSequence16Seq (int initialMaximum) {
        super(ComponentInfoSequence16.class, initialMaximum);
    }

    public ComponentInfoSequence16Seq (Collection elements) {
        super(ComponentInfoSequence16.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ComponentInfoSequence16.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ComponentInfoSequence16 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(ComponentInfoSequence16.create());
