...
/**
* A sequence of MobilitySettingsState instances.
*/
public final class MobilitySettingsStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MobilitySettingsStateSeq() {
        super(MobilitySettingsState.class);
    }

    public MobilitySettingsStateSeq (int initialMaximum) {
        super(MobilitySettingsState.class, initialMaximum);
    }

    public MobilitySettingsStateSeq (Collection elements) {
        super(MobilitySettingsState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MobilitySettingsState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MobilitySettingsState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MobilitySettingsState.create());
