...
/**
* A sequence of ArmState instances.
*/
public final class ArmStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public ArmStateSeq() {
        super(ArmState.class);
    }

    public ArmStateSeq (int initialMaximum) {
        super(ArmState.class, initialMaximum);
    }

    public ArmStateSeq (Collection elements) {
        super(ArmState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, ArmState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' ArmState objects (beyond the original size of this object)
