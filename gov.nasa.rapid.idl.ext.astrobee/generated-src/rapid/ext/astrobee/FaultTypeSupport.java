...
public class FaultTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "rapid::ext::astrobee::Fault";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final FaultTypeSupport _singleton
    = new FaultTypeSupport();
    ...
    */
    public static FaultTypeSupport get_instance() {
        return _singleton;
    }

    public static FaultTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return FaultTypeCode.VALUE;
    }
    ...
    * <code>this</code> is not a <code>Fault</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        Fault typedDst = (Fault) destination;
        Fault typedSrc = (Fault) source;
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            dst.writeInt(typedSrc.code);

            dst.writeString(typedSrc.message,128);

            rapid.KeyTypeValueSequence8TypeSupport.get_instance().serialize(endpoint_data, typedSrc.data, dst, false, encapsulation_id,true,endpoint_plugin_qos);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            Fault typedDst = (Fault) dst;
            typedDst.clear();      
            try{
                typedDst.timestamp = src.readLong();
                typedDst.code = src.readInt();
                typedDst.message = src.readString(128);
                typedDst.data = (rapid.KeyTypeValueSequence8)rapid.KeyTypeValueSequence8TypeSupport.get_instance().deserialize_sample(endpoint_data, typedDst.data, src, false, true, endpoint_plugin_qos);     
    ...
    int mask) {
        return new FaultDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new FaultDataReader(native_reader, listener, mask, this);   
    ...
    protected FaultTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME, false,FaultTypeCode.VALUE,Fault.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected FaultTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,FaultTypeCode.VALUE,Fault.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
