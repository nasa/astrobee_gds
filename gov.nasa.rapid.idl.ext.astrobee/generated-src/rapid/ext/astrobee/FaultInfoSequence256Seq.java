...
/**
* A sequence of FaultInfoSequence256 instances.
*/
public final class FaultInfoSequence256Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FaultInfoSequence256Seq() {
        super(FaultInfoSequence256.class);
    }

    public FaultInfoSequence256Seq (int initialMaximum) {
        super(FaultInfoSequence256.class, initialMaximum);
    }

    public FaultInfoSequence256Seq (Collection elements) {
        super(FaultInfoSequence256.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FaultInfoSequence256.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FaultInfoSequence256 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FaultInfoSequence256.create());
