...
/**
* A sequence of SaveSettingSequence64 instances.
*/
public final class SaveSettingSequence64Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public SaveSettingSequence64Seq() {
        super(SaveSettingSequence64.class);
    }

    public SaveSettingSequence64Seq (int initialMaximum) {
        super(SaveSettingSequence64.class, initialMaximum);
    }

    public SaveSettingSequence64Seq (Collection elements) {
        super(SaveSettingSequence64.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, SaveSettingSequence64.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' SaveSettingSequence64 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(SaveSettingSequence64.create());
