...
/**
* A sequence of DiskInfoConfigSequence8 instances.
*/
public final class DiskInfoConfigSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiskInfoConfigSequence8Seq() {
        super(DiskInfoConfigSequence8.class);
    }

    public DiskInfoConfigSequence8Seq (int initialMaximum) {
        super(DiskInfoConfigSequence8.class, initialMaximum);
    }

    public DiskInfoConfigSequence8Seq (Collection elements) {
        super(DiskInfoConfigSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiskInfoConfigSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiskInfoConfigSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DiskInfoConfigSequence8.create());
