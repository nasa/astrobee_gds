...
public class BerthStateTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "rapid::ext::astrobee::BerthState";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final BerthStateTypeSupport _singleton
    = new BerthStateTypeSupport();
    ...
    */
    public static BerthStateTypeSupport get_instance() {
        return _singleton;
    }

    public static BerthStateTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return BerthStateTypeCode.VALUE;
    }

    public Object create_data() {
        return BerthState.create();
    }
    ...
    * <code>this</code> is not a <code>BerthState</code>
    * type.
    */
    public Object copy_data(Object destination, Object source) {

        BerthState typedDst = (BerthState) destination;
        BerthState typedSrc = (BerthState) source;
    ...
    public long get_serialized_sample_size(
        ...
        Object sample) 
    {
        BerthState typedSrc = (BerthState) sample;
        long origAlignment = currentAlignment;
        long encapsulation_size = currentAlignment;
        ...
        currentAlignment  +=  CdrPrimitiveType.BOOLEAN.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.getStringSerializedSize(currentAlignment , typedSrc.astrobeeName );
        currentAlignment  +=  CdrPrimitiveType.BOOLEAN.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.SHORT.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment);
        currentAlignment  +=  CdrPrimitiveType.INT.getMaxSizeSerialized(currentAlignment);
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            dst.writeBoolean(typedSrc.occupied);

            dst.writeString(typedSrc.astrobeeName,32);

            dst.writeBoolean(typedSrc.awake);

            dst.writeShort(typedSrc.numBatteries);

            dst.writeInt(typedSrc.maxCapacity);

            dst.writeInt(typedSrc.currentCapacity);
        }
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            BerthState typedDst = (BerthState) dst;
            typedDst.clear();      
            try{
                typedDst.occupied = src.readBoolean();
                typedDst.astrobeeName = src.readString(32);
                typedDst.awake = src.readBoolean();
                typedDst.numBatteries = src.readShort();
                typedDst.maxCapacity = src.readInt();
                typedDst.currentCapacity = src.readInt();
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if (deserialize_key) {
            ...
            BerthState typedDst = (BerthState) sample;
    ...
    int mask) {
        return new BerthStateDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new BerthStateDataReader(native_reader, listener, mask, this);   
    ...
    protected BerthStateTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME, false,BerthStateTypeCode.VALUE,BerthState.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected BerthStateTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,BerthStateTypeCode.VALUE,BerthState.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
