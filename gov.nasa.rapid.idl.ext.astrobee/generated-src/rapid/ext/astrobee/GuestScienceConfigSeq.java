...
/**
* A sequence of GuestScienceConfig instances.
*/
public final class GuestScienceConfigSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GuestScienceConfigSeq() {
        super(GuestScienceConfig.class);
    }

    public GuestScienceConfigSeq (int initialMaximum) {
        super(GuestScienceConfig.class, initialMaximum);
    }

    public GuestScienceConfigSeq (Collection elements) {
        super(GuestScienceConfig.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GuestScienceConfig.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GuestScienceConfig objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GuestScienceConfig.create());
