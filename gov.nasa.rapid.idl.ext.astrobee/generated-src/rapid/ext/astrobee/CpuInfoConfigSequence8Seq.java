...
/**
* A sequence of CpuInfoConfigSequence8 instances.
*/
public final class CpuInfoConfigSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public CpuInfoConfigSequence8Seq() {
        super(CpuInfoConfigSequence8.class);
    }

    public CpuInfoConfigSequence8Seq (int initialMaximum) {
        super(CpuInfoConfigSequence8.class, initialMaximum);
    }

    public CpuInfoConfigSequence8Seq (Collection elements) {
        super(CpuInfoConfigSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, CpuInfoConfigSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' CpuInfoConfigSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(CpuInfoConfigSequence8.create());
