...
/**
* A sequence of GuestScienceDataType instances.
*/
public final class GuestScienceDataTypeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GuestScienceDataTypeSeq() {
        super(GuestScienceDataType.class);
    }

    public GuestScienceDataTypeSeq (int initialMaximum) {
        super(GuestScienceDataType.class, initialMaximum);
    }

    public GuestScienceDataTypeSeq (Collection elements) {
        super(GuestScienceDataType.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GuestScienceDataType.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GuestScienceDataType objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GuestScienceDataType.create());
