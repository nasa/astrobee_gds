...
/**
* A sequence of BatteryInfoConfigSequence4 instances.
*/
public final class BatteryInfoConfigSequence4Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BatteryInfoConfigSequence4Seq() {
        super(BatteryInfoConfigSequence4.class);
    }

    public BatteryInfoConfigSequence4Seq (int initialMaximum) {
        super(BatteryInfoConfigSequence4.class, initialMaximum);
    }

    public BatteryInfoConfigSequence4Seq (Collection elements) {
        super(BatteryInfoConfigSequence4.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BatteryInfoConfigSequence4.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BatteryInfoConfigSequence4 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BatteryInfoConfigSequence4.create());
