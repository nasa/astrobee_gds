...
/**
* A sequence of GuestScienceCommand instances.
*/
public final class GuestScienceCommandSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GuestScienceCommandSeq() {
        super(GuestScienceCommand.class);
    }

    public GuestScienceCommandSeq (int initialMaximum) {
        super(GuestScienceCommand.class, initialMaximum);
    }

    public GuestScienceCommandSeq (Collection elements) {
        super(GuestScienceCommand.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GuestScienceCommand.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GuestScienceCommand objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GuestScienceCommand.create());
