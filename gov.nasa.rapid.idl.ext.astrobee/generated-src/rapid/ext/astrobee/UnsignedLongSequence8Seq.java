...
/**
* A sequence of UnsignedLongSequence8 instances.
*/
public final class UnsignedLongSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public UnsignedLongSequence8Seq() {
        super(UnsignedLongSequence8.class);
    }

    public UnsignedLongSequence8Seq (int initialMaximum) {
        super(UnsignedLongSequence8.class, initialMaximum);
    }

    public UnsignedLongSequence8Seq (Collection elements) {
        super(UnsignedLongSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, UnsignedLongSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' UnsignedLongSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(UnsignedLongSequence8.create());
