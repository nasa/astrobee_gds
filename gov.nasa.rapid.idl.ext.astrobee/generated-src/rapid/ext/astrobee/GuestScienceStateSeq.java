...
/**
* A sequence of GuestScienceState instances.
*/
public final class GuestScienceStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GuestScienceStateSeq() {
        super(GuestScienceState.class);
    }

    public GuestScienceStateSeq (int initialMaximum) {
        super(GuestScienceState.class, initialMaximum);
    }

    public GuestScienceStateSeq (Collection elements) {
        super(GuestScienceState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GuestScienceState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GuestScienceState objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GuestScienceState.create());
