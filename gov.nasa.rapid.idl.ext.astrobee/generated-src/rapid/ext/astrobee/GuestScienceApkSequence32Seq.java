...
/**
* A sequence of GuestScienceApkSequence32 instances.
*/
public final class GuestScienceApkSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public GuestScienceApkSequence32Seq() {
        super(GuestScienceApkSequence32.class);
    }

    public GuestScienceApkSequence32Seq (int initialMaximum) {
        super(GuestScienceApkSequence32.class, initialMaximum);
    }

    public GuestScienceApkSequence32Seq (Collection elements) {
        super(GuestScienceApkSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, GuestScienceApkSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' GuestScienceApkSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GuestScienceApkSequence32.create());
