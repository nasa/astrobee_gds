...
/**
* A sequence of InertialProperties instances.
*/
public final class InertialPropertiesSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public InertialPropertiesSeq() {
        super(InertialProperties.class);
    }

    public InertialPropertiesSeq (int initialMaximum) {
        super(InertialProperties.class, initialMaximum);
    }

    public InertialPropertiesSeq (Collection elements) {
        super(InertialProperties.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, InertialProperties.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' InertialProperties objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InertialProperties.create());
