...
/**
* A sequence of MachineInfoSequence8 instances.
*/
public final class MachineInfoSequence8Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public MachineInfoSequence8Seq() {
        super(MachineInfoSequence8.class);
    }

    public MachineInfoSequence8Seq (int initialMaximum) {
        super(MachineInfoSequence8.class, initialMaximum);
    }

    public MachineInfoSequence8Seq (Collection elements) {
        super(MachineInfoSequence8.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, MachineInfoSequence8.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' MachineInfoSequence8 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MachineInfoSequence8.create());
