...
/**
* A sequence of EpsState instances.
*/
public final class EpsStateSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public EpsStateSeq() {
        super(EpsState.class);
    }

    public EpsStateSeq (int initialMaximum) {
        super(EpsState.class, initialMaximum);
    }

    public EpsStateSeq (Collection elements) {
        super(EpsState.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, EpsState.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' EpsState objects (beyond the original size of this object)
