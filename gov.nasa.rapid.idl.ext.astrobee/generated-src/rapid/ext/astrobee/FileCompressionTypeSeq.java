...
/**
* A sequence of FileCompressionType instances.
*/
public final class FileCompressionTypeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public FileCompressionTypeSeq() {
        super(FileCompressionType.class);
    }

    public FileCompressionTypeSeq (int initialMaximum) {
        super(FileCompressionType.class, initialMaximum);
    }

    public FileCompressionTypeSeq (Collection elements) {
        super(FileCompressionType.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, FileCompressionType.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' FileCompressionType objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(FileCompressionType.create());
