...
/**
* A sequence of BooleanSequence32 instances.
*/
public final class BooleanSequence32Seq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public BooleanSequence32Seq() {
        super(BooleanSequence32.class);
    }

    public BooleanSequence32Seq (int initialMaximum) {
        super(BooleanSequence32.class, initialMaximum);
    }

    public BooleanSequence32Seq (Collection elements) {
        super(BooleanSequence32.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, BooleanSequence32.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' BooleanSequence32 objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(BooleanSequence32.create());
