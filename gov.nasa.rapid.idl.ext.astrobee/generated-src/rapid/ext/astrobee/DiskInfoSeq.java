...
/**
* A sequence of DiskInfo instances.
*/
public final class DiskInfoSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public DiskInfoSeq() {
        super(DiskInfo.class);
    }

    public DiskInfoSeq (int initialMaximum) {
        super(DiskInfo.class, initialMaximum);
    }

    public DiskInfoSeq (Collection elements) {
        super(DiskInfo.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, DiskInfo.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' DiskInfo objects (beyond the original size of this object)
