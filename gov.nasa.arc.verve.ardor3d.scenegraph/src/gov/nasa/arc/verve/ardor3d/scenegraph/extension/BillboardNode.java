 ...
 ******************************************************************************/
package gov.nasa.arc.verve.ardor3d.scenegraph.extension;
...
public class BillboardNode extends Node {
    ...
    private final Vector3 _vector = new Vector3(Vector3.UNIT_Z);
    
    private boolean _doUpdate = true;

    public enum BillboardAlignment {
        ScreenAligned, CameraAligned, AxialY, AxialZ, AxialVector
    }
    ...
    public void rotateBillboard() {
        ...
        switch (_alignment) {
            ...
            case AxialZ:
                ...
                break;
            case AxialVector:
            	rotateArbitraryUpVector(_vector);
            	break;
        }
        ...
        propagateDirtyDown(EnumSet.of(DirtyType.Transform));
     ...
     *            Camera
     */
    private void rotateArbitraryUpVector(final Vector3 up) {
        final Camera camera = Camera.getCurrentCamera();
        // Compute the additional rotation required for the billboard to face
        // the camera. To do this, the camera must be inverse-transformed into
        // the model space of the billboard.
        _look.set(camera.getLocation()).subtractLocal(_worldTransform.getTranslation());
        final Matrix3 worldMatrix = Matrix3.fetchTempInstance();
        ...
        _look.normalize(zAxis);
        up.normalize(xAxis).crossLocal(zAxis);
        zAxis.cross(xAxis, yAxis);
        zAxis.normalizeLocal();

        worldMatrix.fromAxes(xAxis, yAxis, zAxis);
    ...
    public void setAxialVector(Vector3 vec) {
    	_vector.set(vec);
    }
