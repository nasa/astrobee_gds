...
public class NewHandler {
	...
	@Execute
	public void execute(MApplication m_application) {
		System.out.println(this.getClass().getSimpleName() 
			      + " @Execute method called");

		IEclipseContext context = m_application.getContext();
		
		PlanFileManager manager = context.get(PlanFileManager.class);
		if(manager != null) {
			manager.onNewPlanCommand();
		}
		LiveTelemetryView verveView = context.get(LiveTelemetryView.class);
		if(verveView != null) {
			verveView.reset();
		}
	}
	...
	public boolean canExecute(EPartService eps) {
		return (boolean) context.get(ContextNames.NEW_PLAN_ENABLED);
	}
}
