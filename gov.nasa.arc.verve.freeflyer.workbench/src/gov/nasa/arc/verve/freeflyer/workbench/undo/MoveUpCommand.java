...
public class MoveUpCommand extends ReversibleCommand {
	...
	public MoveUpCommand(ModuleBayPlan mbp, Sequenceable seq) {
		original = seq;
		plan = mbp;
	}
	...
	public Sequenceable runCommand() {
		...
		PlanEditsRegistry.onMoveUp(moved);
		try {
			clone = moved.clone();
		} catch (CloneNotSupportedException e) {
	...
	public Sequenceable undoCommand() {
		...
		if(moved instanceof Station) {
			DeselectListenerRegistry.onDeselect(); // so the AbstractPlanTrace doesn't get messed up
			plan.moveThisStationDown((Station)moved);
		}
		else if(moved instanceof FreeFlyerCommand) {
			Station parentStation = (Station)moved.getParent();
			original = parentStation.moveThisCommandDown(moved);
		}
		PlanEditsRegistry.onMoveDown(original);
		return moved;
	}
